...
1. `git mv pending/stale/stale.yml .github/workflows/stale.yml`.
2. Delete `pending/stale/`.

The `tests/` tree and the shape test that used to host this file were removed
in the no-glue wipe (fleet-ops#7828), so nothing shape-locks the parked source
today.