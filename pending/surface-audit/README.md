...
1. `git mv pending/surface-audit/reusable-surface-audit.yml .github/workflows/reusable-surface-audit.yml`.
...
3. Optionally: tag a `v1` ref. Until then, callers using `@v1` fail to
   resolve and the gate stays self-skipped everywhere — the safe default.
4. Delete `pending/surface-audit/`.

The `tests/` tree and the shape test that used to lock the parked source were
removed in the no-glue wipe (fleet-ops#7828).
