...
- A failed command (non-zero exit, isError, timeout, GraphQL/HTTP error) is ALWAYS flagged, never walked past: name it in your user-facing text in the same turn before continuing. One line is enough: 'the X call failed with Y, fixing' or 'the X call failed with Y, it is now the blocker.' Burying it in a tool result, a `thinking` block, a comment inside the next toolCall's command, or a silent retry does not count; if it blocks the work, exit nonzero. Cause-explaining prose ("the file was archived", "the text is already the same") names the CAUSE, not the FAILURE, and is not a flag. This covers every toolResult shape: `read` returning ENOENT/EACCES/`Offset N is beyond end of file`/EISDIR; `edit` returning unmatch/multi-match/no-op/schema-validation errors; `bash` exit != 0, `Command timed out`; `gh api`/`gh issue view`/`gh pr view`/`gh pr list` returning 4xx/5xx or `Unknown JSON field`; `git` fatal/128 (checkout worktree-conflict, branch-force, cherry-pick-empty); `python3` tracebacks (KeyError/NameError/ModuleNotFoundError — note hyphenated module names); `systemctl --user status` of a failed unit (exit 3); compound chains whose silenced tail masks an earlier non-zero command; long-running scripts that exit non-zero or time out on partial completion (name the gap, e.g. "timed out at 17/35"). NOT a failure: a no-match probe — `grep`/`rg`/`diff`/`ls`/`which` returning exit 1 with no error text (and `ls` exit 2 is benign). A `read`/`edit`/`cat`/`cd`/`git clone`/`python3 -c`/`systemctl status`/debug-script failure is NEVER a no-match probe. The authoritative enumeration of every pinned shape lives in `tests/fleet-failed-command-*.test.sh` and is enforced by the `bin/fleet-failed-command-flagged` session-close lint, which auto-files `fix(failed-command):` issues for any walked-past failure — when in doubt, flag it. Live stale-path case (fleet-ops#1097): a bare `cat` of `tooling/fleet-ops/bin/fleet-failed-command-flagged` (the `tooling/fleet-ops` checkout does not carry that bin — the canonical path is `tooling/fleet-ops-deploy-clone/bin/...`) returns `cat: <path>: No such file or directory` + exit 1; that is a REAL failure (a `cat` ENOENT is never a no-match probe), and a later thinking-only recovery or silent `find` does not discharge it — name the failure in user-facing text. Live racing-`cd` case (fleet-ops#1217): a `cd /tmp/fleet-ops-fresh-1165 2>/dev/null && ls ...` that races with an in-flight `git clone` into the same path returns `(no output) Command exited with code 1` — the clone is still running, `cd` fails, stderr is silenced; that is a real swallowed failure, not a no-match probe, and a silent `cd && ls` recovery does not discharge it — name it. Live read-ENOENT case (fleet-ops#953): a `read` returning `ENOENT: no such file or directory, access '<path>'` with `isError=true` is a REAL failure (a `read` ENOENT is never a no-match probe), and a later `thinking`-only recovery, a successful `ls` of a sibling path, or "Now I have the full picture" prose that does not name the failure does not discharge it — name the failure in user-facing text. Live stale-checkout read-ENOENT case (fleet-ops#1001): a `read` of `/home/nish/workspaces/fleet-ops-sync/bin/fleet-escalation-canary` (a stale, non-canonical checkout that does not carry the file) returning `ENOENT: no such file or directory, access '<path>'` is the same swallowed-failure class as #953; a successful `ls ~/.local/bin/*canary*` that returns the canonical symlink in the same turn does not discharge the read-ENOENT of the wrong file — name the failure.
Skip-then-todos shape (fleet-ops#958): a `read` ENOENT followed by several thinking-only "skip" turns, then a `todo` add + "Now I have the full picture" prose without naming the ENOENT — the prose does not discharge the failure; name it in user-facing text. Archived-packet read-ENOENT case (fleet-ops#1059): a `read` of `/home/nish/.local/state/pi-issues/fleet-ops-938.in` (since archived to `ARCHIVED-fleet-ops-938.in-20260827T071649Z` by the reap ladder before the session started) returns `ENOENT: no such file or directory, access '<path>'` — cause-explaining prose ("the file was archived") does NOT discharge it; the flag must name the FAILURE (read ENOENT), not the cause, and a `cat` of the archived copy is a follow-up read, not a discharge of the original failure. Read-EISDIR case (fleet-ops#1170 and fleet-ops#1243): a `read` returning `EISDIR: illegal operation on a directory, read` with `isError=true` is a REAL failure (a `read` EISDIR is never a no-match probe), and thinking-only recovery does not discharge it — name it; the live #1170 path is `/home/nish/workspaces/products/0509-work/e2e/fixtures` and the live #1243 sibling (`01a043ee`) also walked past an EISDIR with "printStackTrace threshold setting you mentioned" prose — that prose is cause/musing, not a failure flag. Stale-rg salvage-scan read-ENOENT case (fleet-ops#1255): a `read` of `salvage-secret-scan` under `fleet-ops-rg` (a stale, non-canonical checkout that does not carry the file) returns `ENOENT: no such file or directory, access '<path>'`; thinking-only "other-checkout cause" prose ("the salvage scan ran in a different checkout") does NOT discharge the read-ENOENT — name the failure. Empty-tool-name case (fleet-ops#1242): a malformed Pi toolCall with an empty name returns `Tool  not found` (isError=true, two-space fingerprint because the name is empty) — a real swallowed failure, not a grep/rg/which no-match probe; do not treat the HTTP 400 `Tool name must be nonempty` errorMessage on the next turn as a user-facing flag (it is harness metadata). Pinned in `tests/fleet-failed-command-empty-tool-name.test.sh`. Cherry-pick empty case (fleet-ops#1065): `git cherry-pick <sha>` exits 1 with `previous cherry-pick is now empty` — a real swallowed failure, not a no-match probe; name it as `the git cherry-pick failed with "previous cherry-pick is now empty", fixing`. Pinned in `tests/fleet-failed-command-git-cherry-pick-empty.test.sh`.
Edit-failure family (fleet-ops#956, #965, #1053, #1139, #1173, #1286): `edit` returning "Could not find the exact text in <path>...", "Found N occurrences of the text in <path>. The text must be unique." (fleet-ops#1053), "Could not find edits[0] in <path>..." (fleet-ops#1173), "No changes made to <path>. The replacement produced identical content..." (fleet-ops#1139; live "The text is already the same" cause-prose vs AUDITOR-LOG.md), or `Validation failed for tool "edit": - path: must have required properties path` (fleet-ops#1286; "The edit tool requires the path field" cause-prose) — all real swallowed failures; cause-prose does not discharge. Compound-ls Permission-denied (fleet-ops#1061): compound `;`-chain, downstream `ls` `Permission denied`, silenced tail, exit 1 — not the ls no-match exit-2 exemption.
Python-traceback family (fleet-ops#957, #966, #1003, #1019): `python3 -c`/heredoc crashing with `KeyError: 'input_domain'` or `KeyError: 'comments'` + exit 1 is real (fleet-ops#957, #966); the gh--json+python3 `KeyError: 'comments'` sibling is fleet-ops#1003, #1019 (drain: fleet-failed-command-observe-duplicate-1003.test.sh). Hyphenated-module (fleet-ops#937): `ModuleNotFoundError: No module named '<hyphenated_name>'` + exit 1, same class as #957. PEM-deserialize (fleet-ops#1174): `load_pem_private_key` on a `cat <<'NISHFLEET_PEM_EOF'` env value crashing `Could not deserialize key data` + exit 1; cause-prose does not discharge.
Git-branch-force worktree-ownership (fleet-ops#849, #985): `git branch -f`/`git push --force` inside the worktree holding `<branch>` refuses `fatal: cannot force update the branch` exit 128; name it. Leftover-duplicate citation chains (observe-to-close drains): 01a03e61 read-ENOENT pile fleet-ops#958, #972, #967, #977 (carry fleet-ops#958, #972 and #972, #967 and #967, #977); 01a03dee edit-unmatch pile fleet-ops#965, #970, #975, #980 (carry #965, #970 and #970, #975 and #975, #980); 01a03e38 python-traceback pile fleet-ops#957, #966, #971 (carry fleet-ops#957, #966 and #966, #971).
...
3. Workspace:
   ...
   Never `git clone git@github.com:Nishfleet/fleet-ops.git` — this host has no GitHub SSH key
