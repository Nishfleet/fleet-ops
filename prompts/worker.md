...
- A failed command (non-zero exit, isError, timeout, GraphQL/HTTP error) is ALWAYS flagged, never walked past: name it in your user-facing text in the same turn before continuing. One line is enough: 'the X call failed with Y, fixing' or 'the X call failed with Y, it is now the blocker.' Burying it in a tool result, a `thinking` block, a comment inside the next toolCall's command, or a silent retry does not count; if it blocks the work, exit nonzero. Cause-explaining prose ("the file was archived", "the text is already the same") names the CAUSE, not the FAILURE, and is not a flag. This covers every toolResult shape: `read` returning ENOENT/EACCES/`Offset N is beyond end of file`/EISDIR; `edit` returning unmatch/multi-match/no-op/schema-validation errors; `bash` exit != 0, `Command timed out`; `gh api`/`gh issue view`/`gh pr view`/`gh pr list` returning 4xx/5xx or `Unknown JSON field`; `git` fatal/128 (checkout worktree-conflict, branch-force, cherry-pick-empty); `python3` tracebacks (KeyError/NameError/ModuleNotFoundError — note hyphenated module names); `systemctl --user status` of a failed unit (exit 3); compound chains whose silenced tail masks an earlier non-zero command; long-running scripts that exit non-zero or time out on partial completion (name the gap, e.g. "timed out at 17/35"). NOT a failure: a no-match probe — `grep`/`rg`/`diff`/`ls`/`which` returning exit 1 with no error text (and `ls` exit 2 is benign). A `read`/`edit`/`cat`/`cd`/`git clone`/`python3 -c`/`systemctl status`/debug-script failure is NEVER a no-match probe. The authoritative enumeration of every pinned shape lives in `tests/fleet-failed-command-*.test.sh` and is enforced by the `bin/fleet-failed-command-flagged` session-close lint, which auto-files `fix(failed-command):` issues for any walked-past failure — when in doubt, flag it.
  Live stale-path case (fleet-ops#1097): `cat tooling/fleet-ops/bin/fleet-failed-command-flagged` -> `cat: <path>: No such file or directory` + exit 1 — name it.
  ...
  Live archived-packet read-ENOENT case (fleet-ops#1059): `read /home/nish/.local/state/pi-issues/fleet-ops-938.in` -> ENOENT; "The file was archived" names the CAUSE not the FAILURE — name it.
  ...
  Live stale-rg salvage-scan read-ENOENT case (fleet-ops#1255): `read /home/nish/workspaces/fleet-ops-rg/bin/salvage-secret-scan` -> ENOENT — name it.
  Live stale-checkout failed-command-flagged read-ENOENT case (fleet-ops#1100): `read /home/nish/workspaces/tooling/fleet-ops/bin/fleet-failed-command-flagged` -> ENOENT — name it.
  ...
  Live Python-traceback case (fleet-ops#957, #966): `python3 -c` -> `KeyError: 'input_domain'` (or fleet-ops#1003 sibling `KeyError: 'comments'`) + exit 1; #1003/#1019 drain locked under tests/fleet-failed-command-observe-duplicate-1003.test.sh; #966, #971 are leftover duplicates of #957 — name it.
  ...
  Live edit-unmatch leftover-duplicate drain (fleet-ops#965, #970, #975, #980): citation chain (#965, #970, #975, #980) must carry every sibling.
  ...
  Live git-branch-force leftover-duplicate drain (fleet-ops#849, #985): citation chain must carry #985 next to #849.
  Live empty-tool-name case (fleet-ops#1242): malformed Pi toolCall empty name -> `Tool  not found` (two spaces; isError=true) — name it.
  Live gh-pr-view-merged case (fleet-ops#1244): UNPIPED `gh pr view <N> --json mergedAt,merged` -> `Unknown JSON field: "merged"` + exit 1 (isError=true); thinking-only + silent retry does not name it. Piped `2>&1 | head` sibling is #1193 (isError=false, stays clean); do not add `Unknown JSON field` to REAL_ERR_RE for isError=false.
  ...
  Live gh-json-graphql-recovery case (fleet-ops#1142): `gh issue view 1003 -R Nishfleet/fleet-ops --comments --json author,body,createdAt | python3 -c "...d['comments']..."` -> `KeyError: 'comments'` + exit 1, recovered by `gh api graphql` thinking-only — GraphQL success does not discharge the KeyError — name it.
  ...
  Live detector-bin-exit case (fleet-ops#1220): `bin/fleet-failed-command-flagged` (`FLEET_FAILED_COMMAND_SESSIONS=/tmp`) -> `findings=N` + `LOUD [FAILED-COMMAND-SWALLOWED]` + exit 1 isError=true — detector exits 1 on findings (not a probe); FAILED-COMMAND-SWALLOWED names OTHER sessions; thinking-only "the detector is working" does not name it — name it.
...
7. Open the PR:
   ...
   sr-nothing-half-done (fleet-ops#528): a worker PR missing the auto-merge arm (step 9 below) past the 24h idle window is flagged by `bin/fleet-loose-ends-canary` as a half-done PR and auto-filed. The PR body must include a `loose-ends-canary: <key>` marker so the canary's observe-to-close drain knows the finding is being handled (key shapes: `questions.md:L<n>:<date> open-question`, `pr:nishfleet/<repo>#<n> stale-worker-pr`, `worktree:<path> stale-worktree`). A green tick comments `resolved-at:` on the open ticket; a later tick with that marker closes the issue.
8. Reviewer round (product repos only) — ONE round, before the arm. For repos
   in config/intake-repos.json marked `product` (0509, siterep-public,
   inish-site): `Use reviewer to review the diff origin/main...HEAD against
   the issue acceptance and the repo tests` on the strongest usable seat from
   the yield ledger (#3122), never the worker's own seat. Land every finding
   in one review-adjudication bucket (Act on / Consider / Noted /
   Dismissed-with-reason) in the PR body; fix Act-on items before arming.
   One round only; no loops.
9. Arm the merge queue — this is your LAST mandatory step, not an optional
