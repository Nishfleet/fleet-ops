...
- A failed command (non-zero exit, isError, timeout, GraphQL/HTTP error) is ALWAYS flagged, never walked past: name it in your user-facing text in the same turn before continuing. One line is enough: 'the X call failed with Y, fixing' or 'the X call failed with Y, it is now the blocker.' Burying it in a tool result, a `thinking` block, a comment inside the next toolCall's command, or a silent retry does not count; if it blocks the work, exit nonzero. Cause-explaining prose ("the file was archived", "the text is already the same") names the CAUSE, not the FAILURE, and is not a flag. This covers every toolResult shape: `read` returning ENOENT/EACCES/`Offset N is beyond end of file`/EISDIR; `edit` returning unmatch/multi-match/no-op/schema-validation errors; `bash` exit != 0, `Command timed out`; `gh api`/`gh issue view`/`gh pr view`/`gh pr list` returning 4xx/5xx or `Unknown JSON field`; `git` fatal/128 (checkout worktree-conflict, branch-force, cherry-pick-empty); `python3` tracebacks (KeyError/NameError/ModuleNotFoundError — note hyphenated module names); `systemctl --user status` of a failed unit (exit 3); compound chains whose silenced tail masks an earlier non-zero command; long-running scripts that exit non-zero or time out on partial completion (name the gap, e.g. "timed out at 17/35"). NOT a failure: a no-match probe — `grep`/`rg`/`diff`/`ls`/`which` returning exit 1 with no error text (and `ls` exit 2 is benign). A `read`/`edit`/`cat`/`cd`/`git clone`/`python3 -c`/`systemctl status`/debug-script failure is NEVER a no-match probe. The authoritative enumeration of every pinned shape lives in `tests/fleet-failed-command-*.test.sh` and is enforced by the `bin/fleet-failed-command-flagged` session-close lint, which auto-files `fix(failed-command):` issues for any walked-past failure — when in doubt, flag it.
  Live stale-path case (fleet-ops#1097): a bare `cat` of `tooling/fleet-ops/bin/fleet-failed-command-flagged` (canonical: `tooling/fleet-ops-deploy-clone/bin/...`) → `cat: <path>: No such file or directory` + exit 1; a `cat` ENOENT is never a no-match probe.
  Live racing-`cd` case (fleet-ops#1217): `cd /tmp/fleet-ops-fresh-1165 2>/dev/null && ls ...` racing an in-flight `git clone` → `(no output) Command exited with code 1` with stderr silenced; a silent `cd && ls` recovery does not discharge it.
  Live read-ENOENT case (fleet-ops#953): a `read` returning `ENOENT: no such file or directory, access '<path>'` isError=true is a REAL failure; thinking-only recovery, a sibling `ls`, or "Now I have the full picture" prose does not discharge it.
  Live stale-checkout read-ENOENT case (fleet-ops#1001): a `read` of `/home/nish/workspaces/fleet-ops-sync/bin/fleet-escalation-canary` (stale checkout) → `ENOENT: no such file or directory, access '<path>'`, class #953; a successful `ls ~/.local/bin/*canary*` does not discharge the wrong-path read
  Live read-ENOENT skip-then-todos case (fleet-ops#958): read → `ENOENT: no such file or directory, access '<path>'` isError=true, then thinking-only recovery turns and `todo`-bundled user-facing text — none naming the read — class #953; name it in the turn it happens.
  Live archived-packet read-ENOENT case (fleet-ops#1059): a `read` of `/home/nish/.local/state/pi-issues/fleet-ops-938.in` (the packet was archived) → `ENOENT: no such file or directory, access '<path>'` isError=true, class #953; cause-prose "The file was archived" names the CAUSE, not the FAILURE
  Live read-ENOENT leftover-duplicate drain (fleet-ops#958, #972, #967, #977): #972/#967/#977 are leftover open duplicates of #958 (01a03e61); the citation chain must carry them so an observe-to-close drain does not drop a sibling.
  Live stale-rg salvage-scan read-ENOENT case (fleet-ops#1255): a `read` of `/home/nish/workspaces/fleet-ops-rg/bin/salvage-secret-scan` (stale checkout; real file in fleet-ops-sync) → `ENOENT: no such file or directory, access '<path>'`, class #953; a successful read of the right checkout does not discharge it
  Live stale-checkout failed-command-flagged read-ENOENT case (fleet-ops#1100): a `read` of `/home/nish/workspaces/tooling/fleet-ops/bin/fleet-failed-command-flagged` (stale; canonical `tooling/fleet-ops-deploy-clone/bin/...`) → `ENOENT: no such file or directory, access '<path>'`, class #953/#1001/#1255; thinking-only recovery proves nothing
  Live read-EISDIR case (fleet-ops#1170, fleet-ops#1243): a `read` returning `EISDIR: illegal operation on a directory, read` (read pointed at a directory instead of a file — #1170 the sessions dir, #1243 the sibling reading 0509-work/e2e/fixtures, walked past with "...printStackTrace threshold setting") — a real swallowed failure, not a negative result.
  Live Python-traceback case (fleet-ops#957, #966): `python3 -c`/`python3 << 'EOF'` → `KeyError: 'input_domain'` (or fleet-ops#1003 sibling `KeyError: 'comments'` from a `gh issue view --json ... | python3 -c` pipe) + `Command exited with code 1` — name it; the #1003/#1019 drain is locked under tests/fleet-failed-command-observe-duplicate-1003.test.sh, same shape as the #966, #971 drain.
  Live hyphenated-module case (fleet-ops#937): `python3 -c "from <hyphenated_name> import ..."` → `ModuleNotFoundError: No module named '<hyphenated_name>'` + exit 1, same python-traceback class.
  Live PEM-deserialize case (fleet-ops#1174): `python3 - <<'PY'` calling `load_pem_private_key` on the raw `NISHFLEET_PEM_EOF` heredoc → `ValueError: Could not deserialize key data` + exit 1; cause-prose naming malformed PEM is not a flag
  Live edit-unmatch case (fleet-ops#1053): an `edit` returning `Found N occurrences of the text in <path>. The text must be unique.` (oldText matched multiple locations, e.g. AUDITOR-LOG.md).
  Live no-op edit case (fleet-ops#1139): an `edit` returning `No changes made to <path>. The replacement produced identical content.`; cause-prose "The text is already the same" names the CAUSE, not the FAILURE
  Live edit-array-unmatch case (fleet-ops#1173): the multi-edit variant `Could not find edits[0] in <path>.` is the same edit-failure class.
  Live edit-schema-validation case (fleet-ops#1286): an `edit` rejected before dispatch — `Validation failed for tool "edit": - path: must have required properties path` (isError=true); thinking-only cause-prose "The edit tool requires the path field" names the CAUSE, not the FAILURE
  Live edit-unmatch leftover-duplicate drain (fleet-ops#965, #970, #975, #980): the 01a03dee pile (#956, #965, #970, #975, #980) is locked under tests/fleet-failed-command-observe-duplicate-open.test.sh; the citation chain must carry every sibling.
  Live cherry-pick-empty case (fleet-ops#1065): a `git cherry-pick <sha>` exiting 1 with `The previous cherry-pick is now empty, possibly due to conflict resolution.` (no `fatal:` line).
  Live compound-ls Permission-denied case (fleet-ops#1061): a compound `;`-separated chain where a downstream `ls <path>` fails with `Permission denied` and the chain tail is silenced (bash exits 1, not 2).
  ...
  Live empty-tool-name case (fleet-ops#1242): a malformed Pi toolCall with an empty name returning `Tool  not found` (two spaces; isError=true).
  Live gh-pr-view-merged case (fleet-ops#1244): an UNPIPED `gh pr view <N> --json mergedAt,merged` (or `--json merged`) → `Unknown JSON field: "merged"` + exit 1 (isError=true); a silent `--json title,state` retry does not name it. Piped `2>&1 | head` sibling is #1193 (isError=false, stays clean).
  Live edit-unmatch cat-append case (fleet-ops#1140): an `edit` of `/home/nish/workspaces/agent-state/READY-WORK.md` returning `Could not find the exact text in <path>` (isError=true), recovered by a successful `bash cat >>` append of the SAME path, is still a swallowed failure — a successful write of the same file is recovery, not a flag.
  Live gh-json-graphql-recovery case (fleet-ops#1142): `gh issue view 1003 -R Nishfleet/fleet-ops --comments --json author,body,createdAt | python3 -c "...d['comments']..."` → `KeyError: 'comments'` + exit 1, recovered by a successful `gh api graphql` query with only a thinking "the same bug" note — GraphQL success does not discharge the KeyError.
  Live typo-est-sh case (fleet-ops#1254): `bash tests/fleet-failed-command-edit-unmatch.est.sh` (truncated `.test.sh` typo) → `bash: ...: No such file or directory` + `Command exited with code 127`, no prior harness block, recovered by a silent retry of the real `.test.sh` — the #677 127-ENOENT cascade exemption needs a prior block; a typo is not a probe.
  Live detector-bin-exit case (fleet-ops#1220): `bin/fleet-failed-command-flagged` with `FLEET_FAILED_COMMAND_SESSIONS=/tmp`, `FLEET_FAILED_COMMAND_FILE_ISSUES=0` → `findings=N` + `LOUD [FAILED-COMMAND-SWALLOWED]` + exit 1 (isError=true) — exit on findings is the detector contract (not a probe); FAILED-COMMAND-SWALLOWED names OTHER sessions; thinking-only "the detector is working" does not name it.
- Mechanical-fix rule (fleet-ops#366): a failure is not fixed until its CLASS is mechanically prevented. If this issue is a failure-fix (incident, detector/canary/postmortem bug, revert follow-up), the PR MUST ship a prevention mechanism — a detector, a gate, a regression drill that proves the guard fires, or observe-to-close wiring — or declare `mechanism-impossible: <reason>` in the PR body.
...
Steps:
1. Read the work: `gh issue view <N> -R Nishfleet/<repo> --comments`. (`gh issue view` has no `--body` flag — `unknown flag: --body` (fleet-ops#1055); use `--json body,comments`. `--json` field names must come from the available list (`labels` not `label`); `Unknown JSON field: "label"` is real (fleet-ops#1219). Same class for `gh pr view <N> -R Nishfleet/<repo> --json ...` (fleet-ops#1193): valid field is `mergedAt`, not `merged`; `Unknown JSON field: "merged"` is real, and `2>&1 | head` MASKS the non-zero exit (`head` exits 0, so `isError: false`) — flag the `Unknown JSON field` text anyway. The UNPIPED form returns the same body AND `isError=true` AND `Command exited with code 1` (fleet-ops#1244) — use `--json mergedAt,state`. `gh pr list --sort -mergedAt` is invalid; that silenced value plus `2>/dev/null` yields the live `=== MERGED RECENT ===` + exit-1 shape (fleet-ops#1107).)
