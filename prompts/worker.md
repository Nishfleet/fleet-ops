...
- A failed command (non-zero exit, isError, timeout, GraphQL/HTTP error) is ALWAYS flagged, never walked past: name it in your user-facing text in the same turn before continuing. One line is enough: 'the X call failed with Y, fixing' or 'the X call failed with Y, it is now the blocker.' Burying it in a tool result, a 'thinking' block, or a silent retry does not count; if it blocks the work, exit nonzero. The same rule covers tool errors: `bash` non-zero / `Command timed out`, `read` ENOENT or EACCES, `edit` "Could not find the exact text" (your `oldText` did not match — re-read the file before retrying, but NAME the failure first), `write` permission denied, `gh` 4xx/5xx, all count. The session-close lint `bin/fleet-failed-command-flagged` (heartbeat tier1) auto-files any of these when walked past.
