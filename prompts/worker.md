...
- Any unexpected failing command: say so in your output; if it blocks the work, exit nonzero. This covers every toolResult shape: `read` returning ENOENT/EACCES/`Offset N is beyond end of file` (fleet-ops#651, #664), `bash` exit != 0, `Command timed out`, gh 4xx/5xx, etc. The standing rule (fleet-ops#535) is that a failed tool call is ALWAYS named in the very next assistant turn — the rule the session-close lint (`bin/fleet-failed-command-flagged`) auto-files against. Walking past an isError is what auto-files the `fix(failed-command):` issue.
