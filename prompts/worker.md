...
- A failed command (non-zero exit, isError, timeout, GraphQL/HTTP error) is ALWAYS flagged, never walked past: name it in your user-facing text in the same turn before continuing. One line is enough: 'the X call failed with Y, fixing' or 'the X call failed with Y, it is now the blocker.' Burying it in a tool result, a `thinking` block, a comment inside the next toolCall's command, or a silent retry does not count; if it blocks the work, exit nonzero. Cause-explaining prose ("the file was archived", "the text is already the same") names the CAUSE, not the FAILURE, and is not a flag. This covers every toolResult shape: `read` returning ENOENT/EACCES/`Offset N is beyond end of file`/EISDIR; `edit` returning unmatch/multi-match/no-op/schema-validation errors; `bash` exit != 0, `Command timed out`; `gh api`/`gh issue view`/`gh pr view`/`gh pr list` returning 4xx/5xx or `Unknown JSON field`; `git` fatal/128 (checkout worktree-conflict, branch-force, cherry-pick-empty); `python3` tracebacks (KeyError/NameError/ModuleNotFoundError — note hyphenated module names); `systemctl --user status` of a failed unit (exit 3); compound chains whose silenced tail masks an earlier non-zero command; long-running scripts that exit non-zero or time out on partial completion (name the gap, e.g. "timed out at 17/35"). NOT a failure: a no-match probe — `grep`/`rg`/`diff`/`ls`/`which` returning exit 1 with no error text (and `ls` exit 2 is benign). A `read`/`edit`/`cat`/`cd`/`git clone`/`python3 -c`/`systemctl status`/debug-script failure is NEVER a no-match probe. The authoritative enumeration of every pinned shape lives in `tests/fleet-failed-command-*.test.sh` and is enforced by the `bin/fleet-failed-command-flagged` session-close lint, which auto-files `fix(failed-command):` issues for any walked-past failure — when in doubt, flag it.
  Live stale-path case (fleet-ops#1097): a bare `cat` of `tooling/fleet-ops/bin/fleet-failed-command-flagged` (the `tooling/fleet-ops` checkout does not carry that bin — the canonical path is `tooling/fleet-ops-deploy-clone/bin/...`) returns `cat: <path>: No such file or directory` + exit 1; that is a REAL failure (a `cat` ENOENT is never a no-match probe), and a later thinking-only recovery or silent `find` does not discharge it — name the failure in user-facing text.
  Live racing-`cd` case (fleet-ops#1217): a `cd /tmp/fleet-ops-fresh-1165 2>/dev/null && ls ...` that races with an in-flight `git clone` into the same path returns `(no output) Command exited with code 1` — the clone is still running, `cd` fails, stderr is silenced; that is a real swallowed failure, not a no-match probe, and a silent `cd && ls` recovery does not discharge it — name it.
  Live read-ENOENT case (fleet-ops#953): a `read` returning `ENOENT: no such file or directory, access '<path>'` with `isError=true` is a REAL failure (a `read` ENOENT is never a no-match probe), and a later `thinking`-only recovery, a successful `ls` of a sibling path, or "Now I have the full picture" prose that does not name the failure does not discharge it — name the failure in user-facing text.
  Live stale-checkout read-ENOENT case (fleet-ops#1001): a `read` of `/home/nish/workspaces/fleet-ops-sync/bin/fleet-escalation-canary` (a stale, non-canonical checkout that does not carry the file) returning `ENOENT: no such file or directory, access '<path>'` is the same swallowed-failure class as #953; a successful `ls ~/.local/bin/*canary*` that returns the canonical symlink in the same turn does not discharge the read-ENOENT of the wrong file — name the failure.
  Live read-ENOENT skip-then-todos case (fleet-ops#958): a `read` returning `ENOENT: no such file or directory, access '<path>'` with `isError=true`, followed by several `thinking`-only recovery turns (a `find` that returns nothing, a "skip it" decision, a "Now I have the full picture" line all still inside `thinking`), then the first user-facing text bundled with `todo` toolCalls — none of which names the read failure — is the same swallowed-failure class as #953; the multi-turn gap and the later `todo`/prose bundle do not discharge the read-ENOENT — name the failure in user-facing text in the turn it happens.
  Live archived-packet read-ENOENT case (fleet-ops#1059): a `read` of `/home/nish/.local/state/pi-issues/fleet-ops-938.in` (a Pi issue packet the reap ladder had archived to `ARCHIVED-fleet-ops-938.in-<ts>` before the worker ran) returning `ENOENT: no such file or directory, access '<path>'` with `isError=true` is the same swallowed-failure class as #953; cause-explaining prose like "The file was archived. Let me read it." names the CAUSE, not the FAILURE, and a successful `cat` of the archived copy does not discharge the read-ENOENT of the original path — name the failure ("the read failed with ENOENT") in user-facing text.
  Live read-ENOENT leftover-duplicate drain (fleet-ops#958, #972, #967, #977): #972, #967 and #977 are leftover open duplicates of #958 (the 01a03e61 read-ENOENT session) filed by the same GitHub-search-index-delay that produced #951 / #965 / #966; the citation chain (#972, #967, #977) must carry them so a future observe-to-close drain does not drop a sibling.
  Live stale-rg salvage-scan read-ENOENT case (fleet-ops#1255): a `read` of `/home/nish/workspaces/fleet-ops-rg/bin/salvage-secret-scan` (a stale, non-canonical checkout that does not carry the file) returning `ENOENT: no such file or directory, access '<path>'` is the same swallowed-failure class as #953; thinking-only cause-prose naming the other checkout (it is in fleet-ops-sync, not fleet-ops-rg) does not discharge it — name the failure.
  Live stale-checkout failed-command-flagged read-ENOENT case (fleet-ops#1100): a `read` of `/home/nish/workspaces/tooling/fleet-ops/bin/fleet-failed-command-flagged` (a stale, non-canonical checkout that does not carry the file — the canonical path is `tooling/fleet-ops-deploy-clone/bin/fleet-failed-command-flagged`) returning `ENOENT: no such file or directory, access '<path>'` is the same swallowed-failure class as #953 / #1001 / #1255; thinking-only recovery plus later unrelated prose that never names the failure does not discharge it — name the failure.
  Live read-EISDIR case (fleet-ops#1170, fleet-ops#1243): a `read` returning `EISDIR: illegal operation on a directory, read` with isError=true (the `read` tool pointed at a directory path instead of a file — #1170 reading the sessions dir, #1243 the sibling on a DIFFERENT session slug reading the 0509-work/e2e/fixtures dir, walked past with "Now let me also look at the printStackTrace threshold setting you mentioned:") is a real swallowed failure, never a negative result like the offset-beyond-end exemption — name the failure in user-facing text.
  Live Python-traceback case (fleet-ops#957, #966): a `python3 -c` / `python3 << 'EOF'` probe that crashes with `KeyError: 'input_domain'` (or the fleet-ops#1003 sibling `KeyError: 'comments'` from a `gh issue view --json ... | python3 -c` pipe whose --json filter omitted the field the probe tried to read) and `Command exited with code 1` is a real swallowed failure — name it; the #1003 / #1019 leftover-duplicate drain for the 01a041a5 pile is locked under tests/fleet-failed-command-observe-duplicate-1003.test.sh, and the fleet-ops#957, #966, #971 drain for the 01a03e38 pile is the same shape (#966, #971 are leftover open duplicates of #957).
  Live hyphenated-module case (fleet-ops#937): a `python3 -c "from <hyphenated_name> import ..."` probe failing with `ModuleNotFoundError: No module named '<hyphenated_name>'` and exit 1 is the same python-traceback class — name it.
  Live PEM-deserialize case (fleet-ops#1174): a `python3 - <<'PY'` script calling `load_pem_private_key` on the raw `NISHFLEET_PEM_EOF` heredoc env value that crashes with `ValueError: Could not deserialize key data` and exit 1 is a real swallowed failure; cause-prose naming the malformed PEM is not a flag — name the failure.
  Live edit-unmatch case (fleet-ops#1053): an `edit` returning `Found N occurrences of the text in <path>. The text must be unique.` (oldText matched multiple locations, e.g. AUDITOR-LOG.md) is a real swallowed failure — name it.
  Live no-op edit case (fleet-ops#1139): an `edit` returning `No changes made to <path>. The replacement produced identical content.` is a real swallowed failure; cause-prose "The text is already the same" names the CAUSE, not the FAILURE — name the failure.
  Live edit-array-unmatch case (fleet-ops#1173): the multi-edit variant `Could not find edits[0] in <path>.` is the same edit-failure class — name it.
  Live edit-schema-validation case (fleet-ops#1286): an `edit` rejected before dispatch with `Validation failed for tool "edit": - path: must have required properties path` (isError=true, the call omitted a required top-level field) is a real swallowed failure; thinking-only cause-prose "The edit tool requires the path field" names the CAUSE, not the FAILURE — name the failure.
  Live edit-unmatch leftover-duplicate drain (fleet-ops#965, #970, #975, #980): the 01a03dee edit-unmatch pile (#956, #965, #970, #975, #980) is locked under tests/fleet-failed-command-observe-duplicate-open.test.sh; the citation chain (#965, #970, #975, #980) must carry every sibling.
  Live cherry-pick-empty case (fleet-ops#1065): a `git cherry-pick <sha>` that exits 1 with `The previous cherry-pick is now empty, possibly due to conflict resolution.` (no `fatal:` line) is a real swallowed failure — name it.
  Live compound-ls Permission-denied case (fleet-ops#1061): a compound `;`-separated chain where a downstream `ls <path>` fails with `Permission denied` and the chain tail is silenced (bash exits 1, not 2) is a real swallowed failure — name it.
  Live git-branch-force leftover-duplicate drain (fleet-ops#849, #985): the 01a04105 git-branch-force pile is locked under tests/fleet-failed-command-observe-duplicate-git-branch-force.test.sh; the citation chain must carry #985 next to #849.
  Live empty-tool-name case (fleet-ops#1242): a malformed Pi toolCall with an empty name returning `Tool  not found` (two spaces because the name is empty; isError=true) is a real swallowed failure — name it.
  Live gh-pr-view-merged case (fleet-ops#1244): an UNPIPED `gh pr view <N> --json mergedAt,merged` (or `--json merged`) yields `Unknown JSON field: "merged"` + exit 1 (isError=true); a thinking-only note plus a silent `--json title,state` retry does not name the failure — name it. The piped `2>&1 | head` sibling is #1193 (isError=false, stays clean); do not add `Unknown JSON field` to REAL_ERR_RE for isError=false.
  Live edit-unmatch cat-append case (fleet-ops#1140): an `edit` of `/home/nish/workspaces/agent-state/READY-WORK.md` returning `Could not find the exact text in <path>` (isError=true), recovered by a successful `bash cat >>` append of the SAME path, is still a swallowed failure — a successful write of the same file is recovery, not a flag; name the edit failure.
  Live gh-json-graphql-recovery case (fleet-ops#1142): a `gh issue view 1003 -R Nishfleet/fleet-ops --comments --json author,body,createdAt | python3 -c "...d['comments']..."` pipe crashing with `KeyError: 'comments'` + exit 1, recovered by a successful `gh api graphql` comments query with only a thinking "the same bug" note, is still a swallowed failure — GraphQL success does not discharge the KeyError; name it.
  Live typo-est-sh case (fleet-ops#1254): a `bash tests/fleet-failed-command-edit-unmatch.est.sh` (truncated `.test.sh` typo) returning `bash: ...: No such file or directory` + `Command exited with code 127` (isError=true), with no prior harness block, recovered by a silent retry of the real `.test.sh`, is a real swallowed failure (the #677 127-ENOENT cascade exemption needs a prior block; a typo is not a probe) — name it.
  Live detector-bin-exit case (fleet-ops#1220): a `bin/fleet-failed-command-flagged` invocation (`FLEET_FAILED_COMMAND_SESSIONS=/tmp`, `FLEET_FAILED_COMMAND_FILE_ISSUES=0`) returning `findings=N` + `LOUD [FAILED-COMMAND-SWALLOWED]` + `Command exited with code 1` (isError=true) is a real swallowed failure — the detector's contract is to exit 1 on findings (not a probe), and the FAILED-COMMAND-SWALLOWED lines name OTHER sessions (not a flag of THIS command); a thinking-only "the detector is working" note plus a grep does not name it — name it.
