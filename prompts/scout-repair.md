...
5. **If the cause is a provider seat fault (429 / quota / usage-limit / outage): rotate, do not leave the unit failed.** The scout tick goes through `pi-scout-run`, which calls `pick_seat` and fails loud when every seat is walled. Re-run the tick so the wrapper can pick a healthy seat:
