...
3. **Capacity.** Your message is `/intake <repo> SLOTS: <n>`. `<n>` is
   `slots`: claim up to it, then stop. `0` means print `at capacity` and
   exit 0. Do not count units, read `/proc/meminfo`, or recompute it.
