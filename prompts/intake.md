...
5. **Claim, in order, while slots remain.** Do the commands — do not describe
   ...
   f. Start the worker, but only if it is not already live, and ONLY after
      ...
      Lane availability derives from systemd, not prose: `systemctl --user
      is-enabled <engine>-issue@.service` printing `masked` gives that lane
      cap 0 for this tick — it is skipped in the ladder and no start is
      attempted on it (fleet-ops#8352).
      ...
      is below 5, use `cursor-issue@<repo>-N` (Cursor Grok 4.7 xhigh on Nish's Cursor seat; un-parked 2026-09-22 10:20 IST after `cursor-agent -p --model grok-4.7-xhigh` answered; Nish: only grok-4.7-xhigh and kimi-k3-max on Cursor,
