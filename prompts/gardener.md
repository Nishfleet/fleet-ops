...
2. Correction ladder pass: every memory or standing-rule entry that recurred this week (grep the week's fleet-ops and 0509 issues and PR bodies for its name) is a defect of its rung. For each, file one fleet-ops issue proposing the stock mechanism (lint rule, ruleset, systemd unit property, router config line), label it needs-orchestrator (never agent-ready: agents never admit their own work, fleet-ops#8304; Fable admits).
...
The body has `custom_llm_provider` (`vercel_ai_gateway`), `state` and
