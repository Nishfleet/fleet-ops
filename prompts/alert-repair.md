...
`custom_llm_provider`, always `vercel_ai_gateway` (top-level on every /jev
body — the proxy's budget limiter raises `custom_llm_provider is required`
on pass-through successes without it, fleet-ops#8713);
