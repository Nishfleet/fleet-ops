...
// conditions, rules} (one entry per ruleset) or null if the API fails (403 on
// App-token scope limits, rate limit, transient). The apply path is the only
// consumer — a null means the rule is reported as "skipped-permission" so the
// drift report still observes the situation rather than silent-passing.
...
// LOCAL_RICHER repos (0509, fleet-ops) produce their own gate contexts —
// fleet-ops folds Gitleaks+Semgrep into its "P14 tests" job — so the
// thin-caller contexts are NOT added there: nothing runs them, and a required
// context with no producer deadlocks the merge queue (fleet-ops#5787).
function standardRequiredContexts(repoType, repo) {
  if (repo && isLocalRicher(repo)) return [];
  const ctxs = [];
...
function checkBranchProtection(repo, repoType, settings, exceptions) {
  ...
  const stdCtxs = standardRequiredContexts(repoType, repo);
...
// thin-caller contexts (same source as standardRequiredContexts — skipped
// for LOCAL_RICHER repos, whose own gates already produce their contexts)
// and the repo's existing branch protection + the repo's existing ruleset
...
// was relying on.
export function mergeQueueRequiredContexts(repoType, existingBranchProtectionContexts, existingRulesetContexts, repo) {
  const std = standardRequiredContexts(repoType, repo);
...
function checkMergeQueueRuleset(repo, repoType, settings, exceptions, existingBranchProtectionContexts) {
  ...
  if (fetched == null) {
    findings.push({ rule: "merge-queue-ruleset", status: "skipped-permission", detail: `ruleset "${MERGE_QUEUE_RULESET_NAME}" exists but its rules are unreadable (App-token scope); manual verification required` });
    return findings;
  }
  ...
  const required = mergeQueueRequiredContexts(repoType, existingBranchProtectionContexts, existingRulesetContexts, repo);
