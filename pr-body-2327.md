...
- `bash tests/seat-health-seat-dead.test.sh` -> PASS (D1-D9 + D10-D13;
  D10/D11/D13 now assert the terminal `corpse` class on transient and
  quota corpses and class rebuild on a healthy write).
- `bash tests/seat-lib.test.sh` -> PASS, exit 0 (new invariant 7b:
  seat_usable and the excluded set refuse a stale seat_dead=true corpse).
- Failing-then-passing: the same seat-lib test run against the PRE-fix
  lib (`git show HEAD:lib/seat-lib.sh`) fails with
  `FAIL: corpse: seat_usable must refuse a stale seat_dead=true corpse
  (got usable rc=0)`; against the fixed lib it is green.
- `bash tests/seat-health-classifier.test.sh` PASS,
  ...
  PASS, `tests/pi-audit-run.test.sh` PASS (pi-audit-run touched),
  `tests/seat-lib-degraded.test.sh` / `seat-lib-org-reserve.test.sh` /
  `repair-rotation.test.sh` / `alert-repair-seat-walled.test.sh` /
  `fleet-seat-recovery.test.sh` / `fleet-seat-live-validate.test.sh` /
  `pi-intake-tick-seat-gate.test.sh` / `pi-scout-seat-rotation.test.sh` /
  `manifest-shape.test.sh` / `fleet-unjustified-wait.test.sh` all PASS.
- `sgscan` (security scan of the diff) -> "No new security findings", rc=0.
- crgate: skip locally (CodeRabbit not signed in on this host); the PR's
  CI CodeRabbit review covers it.
- Live state: after the fix, against the LIVE ledger,
  `seat_usable opencode/muse-spark-1.2-contributor-free` returns rc=1
  "UNUSABLE (seat_dead=true, class=corpse)"; `opus-heartbeat-gather` seat
  row for the slug now reads `health_class:"corpse", seat_dead:true`.
- `tests/ci-standards-audit.test.sh` on this worktree reaches the
  ...
  HEAD too — PRE-EXISTING on main, already tracked as
  Nishfleet/fleet-ops#2318 (open); not owned by this issue, not fixed
  here (it needs workflow scope which the worker App cannot push).
