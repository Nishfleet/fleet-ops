...
x-preview-f-free were already cap=0. A control seat on the same provider
was healthy. A 401 on a retired slug is not a re-auth action. Re-auth is
Nish-reserved (account/credential). This PR does not touch auth.json.
...
Out of scope: re-auth of the opencode credential (retired slugs, not a
dead key), rewriting seat-health.ts (out of repo), retiring mimo (it is
still enrolled, cap=1; a 429 is a quota wall, not a corpse).

Related open tickets #3188 and #3366 cover the same mimo never-released
...
```
PI_SEAT_HEALTH_LEDGER_DIR=.run-3301/seats \
  FLEET_SEAT_COMEBACK_STATE=.run-3301/state.json \
  FLEET_SEAT_COMEBACK_PROM=.run-3301/out.prom \
  FLEET_SEAT_COMEBACK_NOW=2026-08-30T12:00:00Z \
  PI_BIN=.run-3301/pi-fail-reanchor-429 \
