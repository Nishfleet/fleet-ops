...
```
$ python3 bin/render-standing-rules.py --check --canonical lib/standing-rules/canonical.md
OK (checked): 2 target(s), 6 section(s)        # exit 0 — branch canonical renders the live CLAUDE.md + codex AGENTS.md byte-identically
...
`bin/fleet-token-efficiency-check` REJECTs `lib/standing-rules/canonical.md` at
line 48 (`{{SURFACE_PREIMPLEMENT_PHRASE}}` inline placeholder) — pre-existing on
origin/main, untouched by this diff (which adds no `{{...}}`), not enforced by any
CI job, and already tracked as a false-positive on the generator-template at
fleet-ops#1813. Not fixed in this PR (out of scope).
