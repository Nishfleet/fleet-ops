...
if command -v node >/dev/null 2>&1 && [ -f "$_deploy_age" ]; then
    ...
    if [ -n "$_deploy_cwd" ]; then
        _deploy_line=$(cd "$_deploy_cwd" && node "$_deploy_age" 2>/dev/null) || _deploy_line=""
    else
        _deploy_line=""
    fi
    if [ -n "$_deploy_line" ] && printf '%s' "$_deploy_line" | grep -q '^deploy: '; then
        printf '%s\n' "$_deploy_line"
