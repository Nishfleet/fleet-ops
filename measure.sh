...
# --- gate-escapes-24h -----------------------------------------------------
# #7501: required failures/cancellations that completed BEFORE merge.
# Extend the existing probe, including per-PR LOUD receipts for the judge.
if [[ -f "$repo_root/lib/required-check-escapes.sh" ]]; then
  # shellcheck disable=SC1091
  source "$repo_root/lib/required-check-escapes.sh"
  read -r -a _ge_repos <<< "$repo_list"
  required_check_escapes_line "${_ge_repos[@]}" || {
    echo 'LOUD required-check-escapes: detector failed'
    echo 'gate-escapes-24h: UNAVAILABLE:detector-failed'
  }
else
  echo 'LOUD required-check-escapes: detector absent'
  echo 'gate-escapes-24h: UNAVAILABLE:detector-absent'
fi
