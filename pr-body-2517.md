...
with the live #2517 shape — replayed fresh this session into an
isolated state dir):

```
$ OPUS_HB_STATE=/tmp/fab-2517-proof.<rand> OPUS_HB_TEXTFILE=/tmp/fab-2517-proof.<rand>/metrics.prom \
    /home/nish/.local/libexec/opus-heartbeat --run-fabricated-scenario <dir>/fab-output.txt
SCENARIO-FABRICATED rc=0 marker=FABRICATED marker=<invoke ft_total=1
exit code: 0

# journal.jsonl
{"ts": "2026-08-31T12:35:30Z", "epoch": 1788179730, "verdict": "FABRICATED_TRANSCRIPT", "claude_rc": 0, "snap_bytes": 0, "marker": "FABRICATED marker=<invoke", "scenario": 1}
...
# actions.log
2026-08-31T12:35:30Z FALLBACK-DISPATCH reason=fabricated-transcript (scenario test)

# run.log
2026-08-31T12:35:30Z FABRICATED-TRANSCRIPT gate=FABRICATED marker=<invoke
2026-08-31T12:35:30Z DONE verdict=FABRICATED_TRANSCRIPT covered=fallback ft_total=1 (scenario)
```

research: the unit-test fixture (tests/opus-heartbeat-fabricated-
transcript-gate.test.sh FAB_OUT) reconstructs the documented live
incident shape from the issue evidence (2026-08-31T10:30:37Z: `<invoke
name="Bash">` + `<parameter>` + invented JSON + invented issue numbers
#2477–2481 + trailing `VERDICT: SMOOTH`). The original 2313-byte
transcript is not preserved on disk — later ticks overwrite
`/home/nish/.local/state/opus-heartbeat/last-claude.txt`, so the test
replays the reported shape rather than a captured byte-string. The
Claude Code `--disallowed-tools` flag (replacing the unreliable
`--tools ""`) was already in use at fleet-ops#1382; the new gate covers
the residual class where the model emits XML anyway. No new `bin/`
files in this PR; the changes ship at the install paths the launcher
and judge prompt already own (both intentionally out-of-repo per the
existing comment block on `/home/nish/.local/libexec/opus-heartbeat`).
