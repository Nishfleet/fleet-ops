...
`pi-issue@*` units run in their own cgroup with `KillMode=control-group`);
load storms are prevented by systemd-oomd (active, fleet-ops#3971) +
CPUWeight/IOWeight (`user-1000.slice` CPUWeight=60, `fleet-work.slice`
CPUWeight=80, `tmux-spawn-*.scope.d` interactive CPUWeight=300). The
