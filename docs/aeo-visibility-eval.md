...
PromptWatch Explore was the only free tool with an API, so Phase B was a card-free signup to confirm the free-tier engine list. The official docs resolve it without a signup (all pages re-fetched live and verified 2026-09-07):

- **Explore (free) tracks ChatGPT only.** Three independent doc pages agree: "the entry-level Explore plan tracks ChatGPT only, and every other plan unlocks all active models" (promptwatch.com/docs/academy/choosing-your-models, repeated verbatim in /docs/academy/no-data-for-model); /docs/academy/plans-and-billing calls Explore "a small prompt set on a single AI model". On Explore the create-monitor form shows other models with an Upgrade badge and won't let you check them. The marketing FAQ ("monitors all AI models") describes the platform, not the free plan; the plan-gating docs are specific and win.
- The issue's bar was **at least two** of {ChatGPT, Perplexity, Claude}. Free covers 1 of 3, so it fails.
- The API exists on Explore (promptwatch.com/docs/rate-limits: 10/s, 150/min burst, 100/hour quota) but is moot: a single-engine tracker cannot replace the in-repo probe, which already covers ChatGPT, Perplexity, and Claude.
