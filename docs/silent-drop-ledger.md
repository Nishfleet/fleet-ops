...
|---|-----------|-----------------|---------------------|-------------|
| 1 | `bin/fleet-blind-audit` (`AUDIT_MAX_FINDINGS` default 1, now 8) | Panel-PASS findings beyond the cap — `skipped: max findings reached` | verdict log only (`REPORT_DIR/verdicts.jsonl`), nothing filed; 2161 findings unqueued since 2026-08-20 | (a) FIXED 2026-09-11 — [fleet-ops#5473](https://github.com/Nishfleet/fleet-ops/pull/5473) + this PR: carry-over ledger `carryover.jsonl`, cap 1→8, reviewer packet uncapped, LOUD `[AUDIT-BACKLOG]` triage line, `--backfill` run once (828 unique reconstructed, 3 filed: #5490/#5492/#5493, rest deduped or carried), canonical-ledger upsert by stable `finding_id` (fleet-ops#5475) |
