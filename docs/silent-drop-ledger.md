...
| 5 | `lib/spec-judge.sh` (6 `gh issue comment` + 3 `gh issue edit` + record-append/relaunch/state-dir sites) | Judge/apply notices: judged markers, judged body edit, block reason, needs-orchestrator/block/absorbed label flips, failure-fallback "spec-judge unavailable" comment — all `\|\| true` | log file only; nothing retryable; fallback loss left the batch claimed unjudged with no record | (a) FIXED [fleet-ops#5438](https://github.com/Nishfleet/fleet-ops/issues/5438) — every gh write now emits `ALERT spec-judge: …` to the tick's stderr on failure; the failure-fallback writes a durable `unavailable-<repo>-<sha>.json` record in the spec-judge state dir BEFORE the comment attempt (decisions-ledger write rejected: `bin/fleet-decisions-ledger` is a lint, `decisions-ledger.md` is a guarded shared file); file moved from DROP_ALLOWLIST to MUST_STAY_CLEAN |
...
| 7 | `scripts/rewrite-0509-author.sh:311,334` | stage8 resume-PR link and stageR rollback-done notices (`\|\| true`) — dropped exactly at the two highest-stakes moments of a history rewrite | `$LOG` file only | (b) QUEUED [fleet-ops#5440](https://github.com/Nishfleet/fleet-ops/issues/5440) |
