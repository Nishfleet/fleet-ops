...
### 3c. Devin CustomLLM sibling (fleet-ops#6228) — RETIRED 2026-09-21

The `fleet_devin_adapter` CustomLLM handler was deleted in the #7907
no-glue sweep: a `custom_provider_map` module is the hand-built extension
class that sweep removed, and `config/litellm-proxy.yaml` no longer names
it. There is no sibling file to install — skip this step entirely; do NOT
symlink anything to `~/.config/fleet-ops/fleet_devin_adapter.py`.

Devin capacity is served by the devin-issue@ worker lane instead
(`systemd/devin-issue@.service` — `devin --print` is itself an agent, not
an HTTP endpoint, so it can never be a LiteLLM router rung; intake picks
`devin-issue@<repo>-N` while fewer than 3 are active). Restore a CustomLLM
bridge only if a stock LiteLLM mechanism ever ships and Nish approves.
