...
Per sample (every 60s) it records:
  ...
  - ready         the live ready-work backlog, read from the fleet-metrics
                  queue-composition cache (fallback: ready-work cache) when it
                  is within READY_FRESH_S, else 0 with ready_source=NO_DATA
  - ready_source  which cache the ready value came from, or NO_DATA
  - ready_age_s   the source cache's age in seconds at sample time
...
# fleet-ops#4956: the #4804 sampler read `data.get("ready", 0)` from
# ready-work-cache.json, whose shape is {"ts": .., "data": 144} — no "ready"
# key — so every sample scored 0 and the decision rule's backlog conjunct was
# unscoreable. Read the real backlog key, in order, from the caches that
# actually carry it. Each entry: (path, key path inside the JSON).
READY_CACHES = (
    # Primordial: {"ts": .., "data": {"ready-work": {"total": N, "self": M}}}
    (OUT_DIR / "queue-composition-cache.json", ("data", "ready-work", "total")),
    # Fallback: {"ts": .., "data": N}
    (OUT_DIR / "ready-work-cache.json", ("data",)),
)
READY_FRESH_S = int(os.environ.get("FLEET_CPU_SAMPLER_READY_FRESH_S", "2400"))
# Why 2400s and not the issue's "~5min": the queue-composition cache is written
# by the fleet-metrics-export 5-min timer, but that write is gated by
# PR_CACHE_TTL = 1800s with "at most one gh fetch per exporter run"
# (libexec/fleet-metrics-export.py `_cached_json`) — the family is only
# refetched once the cache is >30 min old, so its `ts` legitimately ages to
# ~35 min. Proven live 2026-09-10: cache written 19:05:07, the 19:10:00 and
# 19:12:00 export runs both exited 0 and did NOT rewrite it. A 300s window
# therefore read NO_DATA on 4 of 4 live smoke samples — the #4804 failure
# (measured backlog thrown away) in a new costume. 2400s = 30 min producer TTL
# + one 5-min tick + slack; NO_DATA now means the producer is genuinely dead
# or gh has been failing, not that the tick has yet to come round.
...
def _dig(data, keys):
    """Walk keys into nested dicts; return None on any miss or non-dict hop."""
    cur = data
    for key in keys:
...
def _read_ready(now=None):
    """Return (ready, source, age_s) from the freshest cache carrying the key.

    Freshness: the cache's own `ts` must be within READY_FRESH_S (default
    2400s — see the constant). A stale, missing, or key-less cache yields
    (0, "NO_DATA", age) — a scored-zero conjunct, never a silent 0-by-default.
    """
    now = time.time() if now is None else now
    stale_age = None
    for path, keys in READY_CACHES:
        try:
            ...
            continue
        age = now - ts
        if ts <= 0 or age > READY_FRESH_S:
            stale_age = None if stale_age is not None else round(age)
            continue
        value = _dig(data, keys)
        # bool is an int subclass; a true/false is not a backlog count.
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            continue
        return int(value), path.name, round(age)
    return 0, "NO_DATA", stale_age
...
def main():
    ...
    with open(out_path, "a") as f:
        while time.time() < deadline:
            ...
            prev = cur
            ready, ready_source, ready_age = _read_ready()
            rec = {
                "ts": now,
                "load1": _read_load1(),
                "idle_pct": idle_pct,
