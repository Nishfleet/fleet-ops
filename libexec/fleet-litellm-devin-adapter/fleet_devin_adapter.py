...
resolves ``litellm_settings.custom_provider_map`` handlers as a sibling
of the live config, so the deployed copy is
``~/.config/fleet-ops/fleet_devin_adapter.py`` next to
``litellm-proxy.yaml`` and is referenced as
``fleet_devin_adapter.devin_windsurf_llm``. No new unit, timer, or
organ (fleet-ops#1250): the proxy start wrapper already sources
``~/.config/fleet-ops/seats/devin.env``, so ``DEVIN_API_KEY`` is
already in the proxy environment.

Model policy (2026-09-17 recovery, #7761 2026-09-18): only
``swe-2-max`` may route, and only in prose groups (senior / judge /
worker-private). Worker groups must not list this bridge: it cannot
return ``tool_calls`` (the 2026-09-18 outage). ``glm-5-2`` and
``swe-1-7`` are retired; ``swe-2-high`` is parked to 2036.
Configuration cannot widen this allowlist.
