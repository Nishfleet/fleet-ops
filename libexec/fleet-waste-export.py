...
RECEIPT_STEM_RE = re.compile(r"^(?:ARCHIVED-)?(.+?)\.(?:err|out)(?:-.*)?$")
...
def issue_key(unit="", packet_path="", chain_id=""):
    ...
    if name:
        ...
        stem = stem.removeprefix("ARCHIVED-")
...
def load_journal_text(text, start, end, default_lane="unknown"):
    ...
    for i, raw in enumerate(lines):
        ...
        rm = RUN_TS_RE.search(line)
        if rm:
            ts = parse_iso(rm.group(1) + "Z")
            last_ts = ts
        ...
        if ts is None:
            ts = last_ts
        lane = last_lane
        # Live pi-issue-run receipts print PACKET-VERDICT, then the
        # "SUCCESS on lane" / "exited on lane" line. Peek ahead for
        # both the lane and a [ISOZ] stamp the verdict line itself lacks.
        if (not ts) or (not lane) or lane == default_lane:
            for peek in lines[i + 1 : i + 6]:
                if ts is None:
                    prm = RUN_TS_RE.search(peek)
                    if prm:
                        ts = parse_iso(prm.group(1) + "Z")
                        last_ts = ts
                if (not lane) or lane == default_lane:
                    psm = SEAT_LOG_RE.search(peek)
                    if psm:
                        lane = f"{psm.group(1)}/{psm.group(2)}"
                        last_lane = lane
        if not in_window(ts, start, end):
            continue
        events.append(
...
def receipt_unit(path):
    """pi-issue receipt filename -> unit stem (live or ARCHIVED-*.err-<ts>)."""
    name = Path(path).name
    m = RECEIPT_STEM_RE.match(name)
    if m:
        return m.group(1)
    return Path(path).stem.removeprefix("ARCHIVED-")
...
def load_receipt_events(path, start, end):
    ...
    seen = set()
    try:
        for pattern in ("*.out", "*.err", "ARCHIVED-*.out-*", "ARCHIVED-*.err-*"):
    ...
    for fp in sorted(files):
        try:
            ...
            continue
        # A receipt cannot hold an in-window event if it was last written
        # before the window opened. Keeps the 5-min exporter off the full
        # archive.
        if mtime < start:
            continue
        try:
        ...
        events.append(
            _verdict_event(
                ts, lane or "unknown", unit, int(vm.group(1)), vm.group(2), body
            )
        )
    return events
...
def detect_spikes(day_rows):
    ...
    return {
        "empty_run": empty_spike,
        "empty_run_devin": empty_devin,
        "salvage_bleed": salvage_spike,
    }
...
def prove_known_losses(spikes):
    ...
    Devin empty runs (#902) must appear as a tools=0 spike on a devin
    lane. That is NOT required to be the global max empty lane — a
    noisier cursor lane must not hide the #902 class.
    """
    errors = []
    empty = spikes.get("empty_run") or {}
    empty_devin = spikes.get("empty_run_devin") or {}
    salvage = spikes.get("salvage_bleed") or {}
    if empty.get("count", 0) < 3 and empty_devin.get("count", 0) < 3:
        errors.append(
            f"empty-run spike missing or too small: {empty!r} (need count>=3)"
        )
    lane = str(empty_devin.get("lane") or "")
    if empty_devin.get("count", 0) < 3 or "devin" not in lane:
        errors.append(
            f"empty-run spike on a devin lane missing: {empty_devin!r} "
            f"(global max was {empty!r}; need a devin lane with count>=3, fleet-ops#902)"
        )
