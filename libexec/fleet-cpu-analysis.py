...
def _merged_prs(start_ts, end_ts):
    ...
    One gh call per repo (the issue's "one call per repo" rule). Uses the
    REST search surface with a merged:>= filter. Cached to a temp file so a
    re-run does not burn the API budget.
    """
    ...
    if cache.exists() and time.time() - cache.stat().st_mtime < 1800:
        try:
            ...
            pass
    cutoff = time.strftime("%Y-%m-%d", time.gmtime(start_ts))
    result = {}
    try:
        out = subprocess.run(
            ["gh", "search", "prs", "--repo", f"{GH_OWNER}/fleet-ops",
             f"merged:>={cutoff}", "--json", "mergedAt,repository,number",
             "--limit", "500"],
                ...
                if merged:
                    try:
                        ts = int(time.mktime(time.strptime(
                            merged[:19], "%Y-%m-%dT%H:%M:%S")))
                    except ValueError:
                        continue
                    if start_ts <= ts <= end_ts:
                        result.setdefault(repo, []).append(ts)
