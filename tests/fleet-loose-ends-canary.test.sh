...
# Heartbeat-tier1: block 39 wires the canary.
grep -q 'LOOSE_ENDS_CANARY_BIN' "$tier1" \
    || fail "16: heartbeat-tier1 missing LOOSE_ENDS_CANARY_BIN"
grep -q '39. loose-ends canary' "$tier1" \
    || fail "16: heartbeat-tier1 missing block 39 header"
ok "16c: heartbeat-tier1 block 39 wired"
