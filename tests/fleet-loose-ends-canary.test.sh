...
# Hermetic: drop earlier fixtures (issue-77/100 bodies) so the canonical
# run does a clean issue-file create instead of a possible-duplicate comment
# against leftover fake issues (fleet-ops#1212 scoring).
rm -f "$scratch/gh-issues"/issue-*.body "$scratch/gh-issues"/issue-*.comments \
    "$scratch/gh-issues"/issue-*.closed "$scratch/gh-issues/creates" "$scratch/gh-issues/closed"
{
...
n_issue=$(find "$scratch/gh-issues" -maxdepth 1 -name 'issue-*.body' | wc -l)
[[ "$n_issue" -eq 1 ]] \
    || fail "18: canonical run must file exactly one issue (got $n_issue)"
