...
#   unanimous -> intensive loop closed, weekly floor intact, precedence=product
...
audit_run="$repo_root/bin/pi-audit-run"
...
[[ -f "$repo_root/systemd/pi-audit@.service" ]] \
  || fail "missing systemd/pi-audit@.service"
...
grep -q '^ExecStart=/home/nish/.local/bin/fleet-gap-closure-loop$' \
  "$repo_root/systemd/fleet-gap-closure-loop.service" \
  || fail "loop unit ExecStart must be fleet-gap-closure-loop"
...
# #157 weekly floor must remain unconditional.
grep -F -- 'OnCalendar=weekly' "$weekly" >/dev/null \
  || fail "weekly floor timer must stay OnCalendar=weekly"
if grep -qE '^[[:space:]]*(Condition|ExecCondition)' "$weekly"; then
  fail "weekly floor must not gain a Condition* gate"
fi
ok "heartbeat trigger + weekly floor intact"

grep -F -- 'fleet-gap-closure-yield' "$repo_root/prompts/intake.md" >/dev/null \
  || fail "intake.md must call fleet-gap-closure-yield"
grep -F -- 'fleet-gap-closure-order' "$repo_root/prompts/intake.md" >/dev/null \
  || fail "intake.md must call fleet-gap-closure-order"
...
ok "SLO snapshot: #153 placeholder, fills from THROUGHPUT, drill gates green"
...
[[ "$(jq -r '.vote' "$scratch/pkt/out.json")" == "DONE" ]] \
  || fail "pi-audit-run dry-run must write DONE JSON, got $(cat "$scratch/pkt/out.json")"
ok "pi-audit-run dry-run writes a JSON verdict"
