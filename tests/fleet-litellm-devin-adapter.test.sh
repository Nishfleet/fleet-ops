...
# pins: config registration, organ membership, MANIFEST install, docs.
set -euo pipefail
...
assert groups == ["worker-capable", "worker-cheap"], f"devin seats belong to the worker groups, got {groups}"
...
PY
ok "2: config/litellm-proxy.yaml registers devin handler + approved deployments only"

# --- 3. organ membership ---------------------------------------------------
python3 - "$repo_root/config/fleet-organs.json" <<'PY' || fail "3: fleet-organs.json"
import json, sys
organs = {o["name"]: o for o in json.load(open(sys.argv[1]))["organs"]}
files = organs["litellm-proxy"]["files"]
assert "libexec/fleet-litellm-devin-adapter/fleet_devin_adapter.py" in files, files
assert "config/litellm-proxy.yaml" in files
print("organs ok")
PY
ok "3: adapter file lives in the fleet-litellm-proxy organ"

# --- 4. MANIFEST install into the config dir -------------------------------
grep -q 'libexec/fleet-litellm-devin-adapter/fleet_devin_adapter.py[[:space:]]\+/home/nish/.config/fleet-ops/fleet_devin_adapter.py' "$repo_root/MANIFEST" \
    || fail "4: MANIFEST must install the adapter next to the live config (sibling-module loading)"
ok "4: MANIFEST ships adapter to ~/.config/fleet-ops/ (sibling of litellm-proxy.yaml)"

# --- 5. docs ----------------------------------------------------------------
doc="$repo_root/docs/litellm-postgres-setup.md"
grep -q 'fleet_devin_adapter' "$doc" \
    || fail "5: runbook must document the devin adapter install"
grep -q 'custom_provider_map' "$doc" \
    || fail "5: runbook must document custom_provider_map registration"
ok "5: runbook documents adapter install + registration"

# --- 6. no new machinery (fleet-ops#1250) ------------------------------------
if git -C "$repo_root" ls-files systemd/ | grep -qi devin; then
    fail "6: adapter must not add a systemd unit/timer (one handler module only)"
fi
git -C "$repo_root" ls-files | grep -E 'fleet-litellm-devin-adapter' | grep -v 'fleet_devin_adapter.py$' \
    && fail "6: unexpected extra files under the adapter dir"
ok "6: one handler module, no new unit/timer/organ"
