...
#
# Locks the pi-intake@.service drop-in that switches the fleet-ops intake
# tick from the model-based pi-packet-run prompt (prompts/intake.md) to the
# deterministic lib/pi-intake-tick.sh (fleet-ops#1377). Without this drop-in
# the repo installs the model path, whose 50-issue visibility ceiling and
# empty-output no-op failure class starved the fleet (212 ready, 1 claim in
# 2h). The drop-in is the mechanism fix; this test is the prevention — a
# future deploy that drops it (or INSTALL.sh that stops shipping it) fails CI.
set -euo pipefail
