...
#   1. prompts/worker-blocks/d1-gate-integrity.md (the fragment intake
#      assembles into 0509 packets, fleet-ops#3247) keeps the D1 prod migration
#      execution rule needles (senior process gate, never single-agent apply,
#      independent senior blind-review).
#   2. A fragment missing any needle is rejected.
...
worker="$repo_root/prompts/worker-blocks/d1-gate-integrity.md"
...
# Needles from the d1-gate-integrity.md fragment D1 prod migration execution rule.
...
check_worker "$worker" || fail "d1-gate-integrity.md missing one or more D1 prod migration execution needles"
ok "scenario1: d1-gate-integrity.md contains the D1 prod migration execution rule needles"
...
for drop in "${needles[@]}"; do
  ...
  rc=$?
  set -e
  [[ "$rc" -ne 0 ]] || fail "scenario2: d1-gate-integrity.md missing '$drop' should be rejected, got rc=$rc"
...
printf '%s\n' "$mech" | grep -q 'd1-gate-integrity.md' \
  || fail "mechanism must name d1-gate-integrity.md (got: $mech)"
...
printf '%s\n' "$proof" | grep -q 'prompts/worker-blocks/d1-gate-integrity.md' \
  || fail "proof must name the d1-gate-integrity fragment (got: $proof)"
