...
# writer), so a byte-only compare reports false DRIFT-INSTALL forever. Mirrors
# the content_equivalent guard already used by live_newer_than_repo
# (fleet-ops#4894).
...
command -v cmp >/dev/null 2>&1 || fail "test requires cmp"
...
mkdir -p "$work"
repo_cfg="$scratch/seat-caps.json"   # repo copy (src side)
...
printf '{ "providers": { "opencode-go": { "_note": "a b", "cap": 10, "class": "prepaid-quota" } } }\n' >"$repo_cfg"
# Same JSON, but key order differs and an external writer re-escapes the
# space char in _note as \u0020 (mirrors how the live seat-caps rows are
# re-serialized, fleet-ops#4894/#4948). Semantically identical after jq -S.
printf '{ "providers": { "opencode-go": { "class": "prepaid-quota", "_note": "a\\u0020b", "cap": 10 } } }\n' >"$live_cfg"
