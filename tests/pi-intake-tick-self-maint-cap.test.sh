...
_gate_block=$(awk '/Usable seat-slot gate \(fleet-ops#3732\)/{on=1} on{print} on && /^fi$/{n++; if(n==3) exit}' "$tick")
...
_outm=$( unset -f pick_seat; slots=2; eval "$_gate_block"; echo "CLAIM-STEP-REACHED slots=$slots" ) 2>/dev/null || true
