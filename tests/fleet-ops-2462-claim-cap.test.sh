...
# (senior rung). The blocked-on line is now 'orchestrator-seat-exhaustion'
...
grep -qF 'blocked-on: orchestrator-seat-exhaustion' "$tick" \
    || fail "tick does not emit blocked-on: orchestrator-seat-exhaustion on ladder-exhausted skip (fleet-ops#3310)"
ok "Test 4: tick escalates to agent-blocked with blocked-on: orchestrator-seat-exhaustion (ladder exhausted, fleet-ops#3310)"
...
# === Test 7: pi-issue-failed-reap increments reclaim-count on OPEN reap ===
...
ok "Test 7: pi-issue-failed-reap increments reclaim-count on OPEN reap (after cooldown)"
...
# Verify the reset is in the CLOSED branch
_closed_reset=$(awk '/reset reclaim-count/{f=1} f{print NR": "$0; if(/reset reclaim-count.*systemic/) exit}' "$reap")
...
ok "Test 8: pi-issue-failed-reap resets reclaim-count + systemic marker on CLOSED reap"
