...
#   6. Matrix row is enforced; heartbeat §22 is wired; MANIFEST installs.
...
grep -F 'heartbeat_rulebook_redteam_cadence_canary' "$tier1" >/dev/null \
  || fail "tier1 §22 must call heartbeat_rulebook_redteam_cadence_canary"
grep -F 'heartbeat_rulebook_redteam_heading_bonus' "$tier1" >/dev/null \
  || fail "tier1 §22 must call heartbeat_rulebook_redteam_heading_bonus"
grep -F 'rulebook-redteam-cadence.sh' "$tier1" >/dev/null \
  || fail "tier1 §22 must source lib/rulebook-redteam-cadence.sh"
...
ok "matrix enforced, §22 wired, MANIFEST, SANCTIONED_PI_RUNNERS, nested CI host"
