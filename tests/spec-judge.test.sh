...
newbody=$(cat "$SPEC_JUDGE_STATE_DIR/edit-body.txt" 2>/dev/null || true)
[[ "$newbody" == *"depends-on: #2373"* ]] || fail "Test 15c: real depends-on value must survive, got: $newbody"
[[ "$newbody" != "depends-on: #2359"*"\n"* ]] || true
structured=$(printf '%s\n' "$newbody" | grep -E '^depends-on:' || true)
[[ "$structured" == "depends-on: #2373" ]] || fail "Test 15c: structured line changed, got: $structured"
