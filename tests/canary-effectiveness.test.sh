...
fails = [m.Event(organ="fleet-completion-canary", ts=1000.0, kind="failure")]
incs = [
    m.Incident(repo="Nishfleet/fleet-ops", ts=1000.0 + 3600, number=1),
    m.Incident(repo="Nishfleet/fleet-ops", ts=1000.0 + 90000, number=2),
    m.Incident(repo="Nishfleet/fleet-ops", ts=500.0, number=3),  # before any failure
]
...
events = [
    m.Event(organ="fleet-completion-canary", ts=1000.0, kind="run"),
    m.Event(organ="fleet-completion-canary", ts=1000.0, kind="failure"),
    m.Event(organ="siterep-live-canary", ts=1000.0, kind="run"),
    # siterep never failed
]
incidents = [
    m.Incident(repo="Nishfleet/fleet-ops", ts=1000.0 + 1800, number=10),
    m.Incident(repo="Nishfleet/siterep-public", ts=1000.0 + 1800, number=11),
]
by = {s.organ: s for s in m.compute_all(events, incidents)}
assert by["fleet-completion-canary"].caught == 1
assert by["fleet-completion-canary"].missed == 0
assert by["siterep-live-canary"].caught == 0
assert by["siterep-live-canary"].missed == 1
assert abs(by["fleet-completion-canary"].effectiveness_ratio - 1.0) < 1e-9
...
# (completion-canary 2026-08-28T21:52Z,
...
grep -q 'fleet_canary_last_failure_seconds{organ="fleet-resilience-drill"} 1788336000' "$FLEET_CANARY_EFF_OUT" \
  || fail "resilience-drill last_failure wrong: $(grep 'last_failure.*resilience' "$FLEET_CANARY_EFF_OUT" || echo missing)"
...
print(json.dumps({
    "events": [
        {"organ": "fleet-completion-canary", "ts": fail_ts, "kind": "run",
         "detail": "run"},
        {"organ": "fleet-completion-canary", "ts": fail_ts, "kind": "failure",
         "detail": "exit-code"},
    ],
    "incidents": [
        {"repo": "Nishfleet/fleet-ops", "ts": inc_ts, "number": 999901,
...
grep -q 'fleet_canary_effectiveness_ratio{organ="fleet-completion-canary"} 1.000000' "$FLEET_CANARY_EFF_OUT" \
...
try:
    out = tmp / "drill.prom"
    body = m.export_prom([m.OrganStats(organ="fleet-completion-canary")],
    ...
    assert 'fleet_canary_effectiveness_ratio{organ="fleet-completion-canary"} 0.000000' in body
    # Red drill: ok=0 and last_green=0 (stale -> alert fires).
    body0 = m.export_prom([m.OrganStats(organ="fleet-completion-canary")],
