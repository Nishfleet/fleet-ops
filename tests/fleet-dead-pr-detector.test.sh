...
# (fleet-ops#5205): an open PR GitHub reports as `mergeable: CONFLICTING`
# whose conflict has stood past DEAD_PR_STALE_HOURS is stale-conflicting.
#
# THE DEAD CLASS IS DELETED (2026-09-18). The old rule — fleet-ops#4468,
# "a CONFLICTING PR whose resolved parent issue is MERGED/CLOSED is dead"
# — is gone: a closed issue cited by a PR is EVIDENCE, not proof the
# branch cannot land. Live replay: PR #7683's body resolved to the
# unrelated issue #7499, closed 23 minutes earlier, and a live PR was
# published as a dead branch while the merge-landing pass behind the
# ExecStartPre never ran. The stricter signals already cover the real
# class — a dead branch's PR is CLOSED (it leaves this `--state open`
# scan) or its head is deleted on the remote (which closes the PR).
# Cases below therefore assert the SAME parent resolution as before (the
# stale clock still needs a live parent) but NEVER a death verdict: a
# non-OPEN parent is skipped, rc 0.
#
# Parent resolution is deterministic and exact-number only (#11352 never
# matches issue 1135).
#
...
#
# REGRESSION REPLAY: PR #9's body carried "Companion to
# Nishfleet/siterep-public PR #36" — a cross-repo PR reference. The
# generic bounded-`#N` fallback read it as fleet-ops issue #36 (an
# unrelated intake issue). Case 6 pins the fix: #36 is never viewed,
# nothing resolves, the PR is skipped.
#
...
# `mergeable: "UNKNOWN"` (or null). Cases 15-19 pin the contract: any
# value that is not exactly MERGEABLE/CONFLICTING (`"UNKNOWN"`, null, a
# future value) is counted in the `unknown-mergeable` figure, kept in the
# scan, and re-queried with `gh pr view` (up to DEAD_PR_UNKNOWN_ATTEMPTS
# tries). Only a decided answer ends the question; still undecided, the
# PR is logged and skipped without resolving a parent at all — with the
# dead class deleted an undecided value can hide nothing, and the stale
# clock only ever runs on a decided CONFLICTING value.
#
...
# leaves the class (MERGEABLE, undecided, unresolved or non-OPEN parent,
# or gone from the open list) the GC pass drops the marker. Flagged members are
...
#
# Measure line: `stale_conflicting_prs=<n>` is the LAST stdout line.
# Exit codes: 0 clean, 1 when at least one stale-conflicting PR is
# proven, 2 on a gh/jq infra failure that stopped the scan.
...
last_measure() {
    ...
    # `stale_conflicting_prs=<n>` measure line.
    tail -n 1 "$scratch/stdout.log"
}

no_death() {
    # The deleted dead class must leave no trace: no evidence line, no
    # measure, no non-zero exit attributable to a parent's state.
    grep -q '^dead-pr' "$scratch/stdout.log" \
        && fail "$1: dead class is deleted — no dead-pr line may be printed: $(cat "$scratch/stdout.log")"
    grep -q 'dead_conflicting_prs' "$scratch/stdout.log" \
        && fail "$1: dead class is deleted — no dead measure may be printed: $(cat "$scratch/stdout.log")"
    return 0
}

# --- Case 1 (dead class deleted): trailer `Fixes #N` resolves the parent,
# parent CLOSED -> the cited issue is not a live parent, so the PR is
# SKIPPED, rc 0. Resolution itself is still proven by the issue view. ---
set_fixtures \
  '[{"number":9,"title":"feat: cloud upload v2","headRefName":"fix/cloud-upload","mergeable":"CONFLICTING","body":"Re-upload path rebuilt.\n\nFixes #595"}]' \
  595:CLOSED
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "trailer+CLOSED parent must NOT fail the unit: $rc"
grep -q 'issue view 595' "$scratch/gh.log" || fail "must query issue 595: $(cat "$scratch/gh.log")"
no_death case1
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be last stdout line = 0: $(last_measure)"
grep -q 'not a live parent' "$scratch/stderr.log" || fail "non-OPEN parent must be logged as skipped: $(cat "$scratch/stderr.log")"
ok "case1: Fixes #595 + CLOSED parent -> resolved, skipped, exit 0 (no death verdict)"

# --- Case 2: trailer `Closes #N`, parent MERGED -> resolved, skipped ---
set_fixtures \
  '[{"number":46,"title":"feat: seat retry windows","headRefName":"fix/seats","mergeable":"CONFLICTING","body":"## Fix\n\nCloses #1941"}]' \
  1941:MERGED
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "Closes+MERGED parent must NOT fail the unit: $rc"
grep -q 'issue view 1941' "$scratch/gh.log" || fail "must query issue 1941: $(cat "$scratch/gh.log")"
no_death case2
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
ok "case2: Closes #1941 + MERGED parent -> resolved, skipped, exit 0"

# --- Case 3: `Relates to #N`, parent CLOSED -> resolved, skipped ---
set_fixtures \
  '[{"number":87,"title":"feat: escalation exclude","headRefName":"fix/escalate","mergeable":"CONFLICTING","body":"Adjudication notes.\n\nRelates to #2133"}]' \
  2133:CLOSED
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "Relates-to+CLOSED parent must NOT fail the unit: $rc"
grep -q 'issue view 2133' "$scratch/gh.log" || fail "must query issue 2133: $(cat "$scratch/gh.log")"
no_death case3
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
ok "case3: Relates to #2133 + CLOSED parent -> resolved, skipped, exit 0"

# --- Case 4: claim/issue-N head branch resolves the parent; CLOSED -> skipped ---
set_fixtures \
  ...
  1126:CLOSED
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "claim-branch + CLOSED parent must NOT fail the unit: $rc"
grep -q 'issue view 1126' "$scratch/gh.log" || fail "must query issue 1126: $(cat "$scratch/gh.log")"
no_death case4
ok "case4: claim/issue-1126 branch resolves parent 1126; CLOSED -> skipped, exit 0"
...
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
grep -q 'conflict first seen' "$scratch/stderr.log" || fail "OPEN parent starts the stale clock: $(cat "$scratch/stderr.log")"
ok "case5: Relates to #489 + OPEN parent -> live parent, stale clock started, exit 0"
...
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "cross-repo PR ref must not flag the PR: $rc"
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
...
set_fixtures \
  ...
  3111:CLOSED
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "resolution must not fail the unit: $rc"
...
no_death case7
ok "case7: foo/bar#42 scrubbed, fleet-ops#3111 kept -> parent resolved as 3111, never 42"
...
# --- Case 10: mixed sweep — a CLOSED-parent PR and a live-parent PR.
# Both resolve, NEITHER is flagged: the closed parent is skipped and the
# live one only starts its stale clock. ---
set_fixtures \
  '[{"number":9,"title":"feat: cloud upload v2","headRefName":"fix/cloud-upload","mergeable":"CONFLICTING","body":"Re-upload path rebuilt.\n\nFixes #595"},{"number":21,"title":"feat: gap closure","headRefName":"fix/gap","mergeable":"CONFLICTING","body":"Partial fix.\n\nRelates to #489"}]' \
  595:CLOSED 489:OPEN
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "closed-parent + live-parent must exit 0: $rc"
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
grep -q 'issue view 595' "$scratch/gh.log" || fail "must query issue 595: $(cat "$scratch/gh.log")"
grep -q 'issue view 489' "$scratch/gh.log" || fail "must query issue 489: $(cat "$scratch/gh.log")"
no_death case10
[ -f "$scratch/state/fleet-ops-21.first-seen" ] || fail "case10: the live-parent PR must start its stale clock"
[ -f "$scratch/state/fleet-ops-9.first-seen" ] && fail "case10: a non-OPEN parent must never start a stale clock"
ok "case10: CLOSED parent (595) skipped, OPEN parent (489) clocked -> exit 0, nothing flagged"
...
ok "case11: MERGEABLE PRs only -> stale_conflicting_prs=0, exit 0, no issue views"
...
grep -q 'stale_conflicting_prs' "$scratch/stdout.log" && fail "infra failure must not print a measure (no false green): $(cat "$scratch/stdout.log")"
...
grep -q 'stale_conflicting_prs' "$scratch/stdout.log" && fail "gh missing must not print a measure: $(cat "$scratch/stdout.log")"
...
# with 11352 OPEN and 1135 CLOSED: the detector must view 11352 and never
# 1135 — this case fails the moment resolution touches the substring. ---
set_fixtures \
...
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "exact-number pin: measure must be 0: $(last_measure)"
...
# --- Case 15 (fleet-ops#5079): `gh pr list` says mergeable "UNKNOWN". The
# list fetch is not the last word: the PR is counted in
# `unknown-mergeable`, kept in the scan, and re-queried with `gh pr view`
# (which here answers CONFLICTING). The decided value is what the scan
# then acts on — here the parent is CLOSED, so the PR is skipped. ---
set_fixtures \
...
grep -q '^rc=0$' <<<"$rc" || fail "re-query answered CONFLICTING + CLOSED parent must exit 0: $rc"
grep -q 'pr view 4978' "$scratch/gh.log" || fail "undecided mergeable must be re-queried with gh pr view: $(cat "$scratch/gh.log")"
grep -q 'issue view 4945' "$scratch/gh.log" || fail "a DECIDED conflict must still resolve its parent: $(cat "$scratch/gh.log")"
...
grep -q 'conflicting=1' "$scratch/stderr.log" || fail "the re-queried CONFLICTING value must count: $(cat "$scratch/stderr.log")"
no_death case15
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
ok "case15: list UNKNOWN -> view CONFLICTING, counted as a conflict, CLOSED parent -> skipped, rc 0"

# --- Case 16: the re-query never decides — the list fetch AND every
# `gh pr view` answer "UNKNOWN". No conflict is proven, so the PR is
# logged and skipped WITHOUT resolving a parent at all (the parent has no
# say now that the dead class is deleted). DEAD_PR_UNKNOWN_ATTEMPTS is
# NOT overridden here: the call-count assertion pins the SHIPPED
# default. ---
set_fixtures \
  '[{"number":4978,"title":"feat: intake reconcile","headRefName":"fix/intake","mergeable":"UNKNOWN","body":"Fixes #4945"}]' \
  4945:CLOSED
...
rc=$(run DEAD_PR_UNKNOWN_SLEEP=0)
grep -q '^rc=0$' <<<"$rc" || fail "still-undecided must be skipped, not fail the unit: $rc"
no_death case16
grep -q 'no conflict proven' "$scratch/stderr.log" || fail "the skip must be logged: $(cat "$scratch/stderr.log")"
grep -q 'issue view' "$scratch/gh.log" && fail "an undecided PR must not spend an issue view: $(cat "$scratch/gh.log")"
...
grep -q 'unknown-mergeable=1' "$scratch/stderr.log" || fail "honesty line must still count it: $(cat "$scratch/stderr.log")"
ok "case16: still UNKNOWN after the shipped 3 tries -> logged and skipped, rc 0, no issue view spent"

# --- Case 17: still undecided after the retries, with an OPEN parent in
# the fixtures. The parent is irrelevant now — no conflict is proven, so
# the PR is skipped, no stale clock starts, and the honesty line still
# shows the undecided value so a blind tick is visible in the journal. ---
set_fixtures \
  '[{"number":4978,"title":"feat: intake reconcile","headRefName":"fix/intake","mergeable":"UNKNOWN","body":"Fixes #4945"}]' \
  4945:OPEN
set_pr_view 4978 UNKNOWN
rc=$(run DEAD_PR_UNKNOWN_SLEEP=0)
grep -q '^rc=0$' <<<"$rc" || fail "still-undecided must exit 0: $rc"
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
grep -q 'unknown-mergeable=1' "$scratch/stderr.log" || fail "honesty line must count the undecided value: $(cat "$scratch/stderr.log")"
[ -f "$scratch/state/fleet-ops-4978.first-seen" ] && fail "an undecided PR must not start a stale clock"
no_death case17
ok "case17: still UNKNOWN -> rc 0, logged, honesty line shows it, no stale clock, never paged"

# --- Case 18: an undecided mergeable must not resurrect the bogus-parent
# class (case 6). List UNKNOWN + a cross-repo `PR #36`-style body: the PR
# is logged and skipped — the bogus #36 is never viewed, nothing paged. ---
set_fixtures \
...
rc=$(run DEAD_PR_UNKNOWN_SLEEP=0)
grep -q '^rc=0$' <<<"$rc" || fail "undecided must be skipped, not paged: $rc"
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
grep -q 'issue view 36' "$scratch/gh.log" && fail "bogus parent #36 must never be viewed: $(cat "$scratch/gh.log")"
grep -q 'no conflict proven' "$scratch/stderr.log" || fail "skipped PR must be logged: $(cat "$scratch/stderr.log")"
ok "case18: undecided mergeable + cross-repo PR ref -> skipped, #36 never viewed, never paged"
...
# CONFLICTING), then skipped on the non-OPEN parent. ---
set_fixtures \
  '[{"number":4978,"title":"feat: intake reconcile","headRefName":"fix/intake","mergeable":null,"body":"Fixes #4945"}]' \
  4945:CLOSED
set_pr_view 4978 CONFLICTING
rc=$(run DEAD_PR_UNKNOWN_SLEEP=0)
grep -q '^rc=0$' <<<"$rc" || fail "null mergeable re-queried to CONFLICTING + CLOSED parent must exit 0: $rc"
...
no_death case19
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "measure must be = 0: $(last_measure)"
ok "case19: list mergeable null -> view CONFLICTING, CLOSED parent -> skipped, rc 0"
...
# measure line LAST, the PR on the flagged list for red-pr-repair, and
# the marker kept (the PR is still in class). ---
set_fixtures \
...
rc=$(run)
grep -q '^rc=1$' <<<"$rc" || fail "case20: stale-conflicting must exit 1: $rc"
...
[ "$(last_measure)" = "stale_conflicting_prs=1" ] \
  || fail "case20: stale_conflicting_prs=1 must be the LAST stdout line: $(last_measure)"
no_death case20
...
[ "$(last_measure)" = "stale_conflicting_prs=0" ] \
  || fail "case21: last measure must be stale_conflicting_prs=0: $(last_measure)"
[ -f "$scratch/state/fleet-ops-4830.first-seen" ] \
...
# same scan — a non-OPEN-parent PR, a MERGEABLE PR, and a vanished PR
# all lose their markers; the flagged list rewrites empty and the sweep
# exits 0 (a CLOSED parent is no longer a verdict). ---
set_fixtures \
...
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "case23: a CLOSED parent must not fail the sweep: $rc"
no_death case23
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "case23: stale measure must be 0: $(last_measure)"
for gone in fleet-ops-9 fleet-ops-66 fleet-ops-999; do
...
ok "case23: GC prunes markers for closed-parent/mergeable/vanished PRs; sweep stays clean"
...
[ "$(last_measure)" = "stale_conflicting_prs=1" ] || fail "case24: stale measure must be 1: $(last_measure)"
...
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "case25: corrupt marker resets inside bound: $(last_measure)"
...
# `#N` mention is an unrelated issue while the house `moves:` delivery
# trailer names the real parent. Priority 1 must win: #5997 is viewed,
# #3322 never is. The trailer keeps its repo qualifier and colon. ---
set_fixtures \
...
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "case26: resolution must not fail the sweep: $rc"
no_death case26
...
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "case27: three delivery-trailer forms must not fail the sweep: $rc"
no_death case27
grep -q 'issue view 1941' "$scratch/gh.log" || fail "case27: moves: #1941 must resolve: $(cat "$scratch/gh.log")"
grep -q 'issue view 2133' "$scratch/gh.log" || fail "case27: Moved #2133 must resolve: $(cat "$scratch/gh.log")"
grep -q 'issue view 595' "$scratch/gh.log" || fail "case27: Fixes: #595 colon form must resolve: $(cat "$scratch/gh.log")"
...
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "case28: measure must be 0: $(last_measure)"
...
[ "$(last_measure)" = "stale_conflicting_prs=1" ] \
  || fail "case29: stale measure must be 1 while the PR is OPEN: $(last_measure)"
...
[ "$(last_measure)" = "stale_conflicting_prs=0" ] \
  || fail "case29: stale measure must be 0 after the close: $(last_measure)"
[ ! -f "$scratch/state/fleet-ops-5834.first-seen" ] \
...
grep -q 'scan done: conflicting=0 unknown-mergeable=0 stale-conflicting=0' "$scratch/stderr.log" \
...
# --- Case 30 (REPLAY of the deleted rule, PR #7683, 2026-09-18): a live
# PR whose body's first `#N` ref is an unrelated issue closed minutes
# earlier. Under fleet-ops#4468 this published a dead-branch verdict and
# failed fleet-merged-pr-close, so the merge-landing pass never ran. The
# rule is deleted: the PR is skipped, the sweep is clean, and the real
# death signals (PR closed / head branch deleted) are left to remove it
# from the `--state open` list on their own. ---
set_fixtures \
  '[{"number":7683,"title":"fix(lifecycle-label-sweep): post spec-gate refusal once per issue (fleet-ops#5890)","headRefName":"fix/specgate-refusal-once","mergeable":"CONFLICTING","body":"Evidence: the backup-sizing pass (#7499) showed the same shape."}]' \
  7499:CLOSED
rc=$(run)
grep -q '^rc=0$' <<<"$rc" || fail "case30: a PR citing a closed issue must not fail the sweep: $rc"
no_death case30
[ "$(last_measure)" = "stale_conflicting_prs=0" ] || fail "case30: measure must be 0: $(last_measure)"
[ ! -f "$scratch/state/fleet-ops-7683.first-seen" ] \
  || fail "case30: a non-OPEN cited issue must not start a stale clock"
ok "case30 (replay): PR #7683 citing CLOSED #7499 -> skipped, sweep clean, landing pass unblocked"
