<<<<<<< HEAD
#!/usr/bin/env bash
# fleet-ops#4263 P3b: listed in ci.yml (workers cannot edit workflows). The quota_bench wall ceiling lived in the deleted routing library; proxy cooldown owns walls now.
...
ok "wall horizon retired with the picker"
||||||| (empty tree)
=======
#!/usr/bin/env bash
...
#     OUT-OF-REPO seat-health extension, which the write-side geometric cap
#     cannot reach; same fence shape as the fleet-ops#2288 transient_fault park).
...
#   H3  read side: an extension-written quota_bench ledger with the live
#       19-day bench_until is UNUSABLE now but USABLE once the horizon
#       passes (the wall no longer outlives the reset cycle).
#   H4  read side: a bench_until inside the horizon still holds the seat.
#   H5  read side: no quota_window -> the 19-day wall is honoured verbatim
#       (the cap never fires where it is not configured).
#   H6  the cap never WIDENS a wall.
#
...
# ---------------------------------------------------------------- H1 / H2
while read -r prov expect; do
    got=$(provider_wall_ceiling_s "$prov")
    [[ "$got" == "$expect" ]] || fail "H1: provider_wall_ceiling_s $prov expected $expect, got '$got'"
done <<EOF
hourlyp 3600
dailyp 86400
cline $WEEK
...
# ---------------------------------------------------------------- H3
# An extension-written marker (the write-side geometric cap cannot reach it):
# observed now, bench_until 19 days out. Held now, released once the horizon
# passes.
now_s=$(date -u +%s)
write_ext_ledger() {
...
if seat_usable cline "cline-pass/minimax-m3" >/dev/null 2>&1; then
    fail "H3: a seat inside the reset horizon must still be unusable"
fi
...
seat_usable cline "cline-pass/minimax-m3" >/dev/null 2>&1 \
    || fail "H3: past the reset horizon the 19-day wall must fail open (re-probe)"
...
if seat_usable cline "cline-pass/minimax-m3" >/dev/null 2>&1; then
    fail "H4: a bench_until inside the horizon must still hold the seat"
fi
...
if seat_usable nowindow m1 >/dev/null 2>&1; then
    fail "H5: with no quota_window the 19-day wall must be honoured verbatim"
fi
...
[[ "$got" == "$short" ]] || fail "H6: cap widened a short wall ($short -> $got)"
got=$(_wall_capped_at_horizon cline "" "$short")
[[ "$got" == "$short" ]] || fail "H6: empty anchor must not widen a short wall (got $got)"
got=$(_wall_capped_at_horizon cline "$(iso_at "$now_s")" "not-a-timestamp")
[[ "$got" == "not-a-timestamp" ]] || fail "H6: unparseable wall must be echoed unchanged (got $got)"
