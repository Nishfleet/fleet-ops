...
# fleet_seat_spend_usd{provider,day} from Prometheus (the gauge fleet-ops#3283
# exports — a per-UTC-day total, so the digest sums today's + yesterday's
# buckets, never delta/rate which would be ~0 on a constant gauge).
#
...
# absent-metric path and asserts the graceful fallback. A third case asserts
# the query sent to Prometheus carries the day=~ matcher for the current and
# previous UTC days (guards the delta() regression that read ~0 forever).
...
env SPEND_RESPONSE="$scratch/spend.json" \
    CURL_QUERY_CAPTURE="$scratch/query1.txt" \
...
# --- case 3: the PromQL sums today+yesterday UTC-day buckets -----------------
# fleet_seat_spend_usd{provider,day} is a per-day gauge; delta/rate over it is
# ~0 forever (reproduced live). The digest must send the day=~ matcher.
today_utc="$(date -u +%F)"
yday_utc="$(date -u -d 'yesterday' +%F)"
[[ -f "$scratch/query1.txt" ]] || fail "case 3: curl stub never saw a /api/v1/query call"
grep -q "fleet_seat_spend_usd" "$scratch/query1.txt" \
  || fail "case 3: query does not reference fleet_seat_spend_usd: $(cat "$scratch/query1.txt")"
grep -qF "day=~\"$today_utc|$yday_utc\"" "$scratch/query1.txt" \
  || fail "case 3: query lacks day=~ matcher for $today_utc|$yday_utc: $(cat "$scratch/query1.txt")"
grep -qF "sum by (provider)" "$scratch/query1.txt" \
