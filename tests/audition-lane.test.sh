...
# Extract the audition functions (lines 581-775) from the intake tick.
# This captures the env-var block + all 3 functions WITHOUT the top-level
# call at line 778 (which would run the tick). We source seat-lib first so
# _seat_caps_loaded and load_seat_caps are available.
audition_funcs="$scratch/audition-funcs.sh"
sed -n '581,783p' "$intake" > "$audition_funcs"
...
ok "promotion at fleet median files promote verdict via fleet-issue-file"
