...
# 4. a crashing detector folds to UNAVAILABLE, never a partial line (the
#    detector's own contract is exit 0; a non-zero here is engine drift)
printf 'process.stdout.write("garbage\\n"); process.exit(1);' >"$scratch/dets/crash.mjs"
out=$(run_deploy_lines "$scratch/dets/crash.mjs")
echo "$out" | grep -q '^deploy: UNAVAILABLE:detector-not-installed$' || fail "crash not flagged: $out"
ok "4. crashing detector -> UNAVAILABLE"
