...
# --- 21. Product-ratio cache freshness check (fleet-ops#2595) ------------
...
grep -qE '^# 36\.5 PRODUCT-RATIO CACHE FRESHNESS' "$tier1" \
    || fail "scenario21: heartbeat-tier1 must wire block 36.5 (product-ratio cache freshness)"
grep -qE 'PRODUCT_RATIO_PROM_PATH|product-ratio\.prom' "$tier1" \
    || fail "scenario21: heartbeat-tier1 must reference the product-ratio prom path"
grep -qE 'PRECEDENCE-BAND-CACHE-STALE' "$tier1" \
    || fail "scenario21: heartbeat-tier1 must emit PRECEDENCE-BAND-CACHE-STALE on staleness"
grep -qE 'PRECEDENCE-BAND-CACHE-MISSING' "$tier1" \
    || fail "scenario21: heartbeat-tier1 must emit PRECEDENCE-BAND-CACHE-MISSING on absence"
grep -qE 'PRODUCT_RATIO_CACHE_FRESHNESS_S' "$tier1" \
    || fail "scenario21: heartbeat-tier1 must read the freshness threshold from PRODUCT_RATIO_CACHE_FRESHNESS_S"
ok "scenario21: heartbeat-tier1 wires the product-ratio cache freshness block 36.5"

# 21a: prove the freshness math under controlled mtime via a sourced
...
set -e
[[ "$rc" == "0" ]] || fail "scenario21a: fresh prom must report OK (rc=0), got rc=$rc out=$out"
[[ "$out" == "FRESH"* ]] || fail "scenario21a: fresh prom must print FRESH line, got $out"
ok "scenario21a: fresh prom (mtime=now) reports FRESH and exits 0"

set +e
out=$(freshness_drive "$prom_stale" 1800); rc=$?
set -e
[[ "$rc" == "1" ]] || fail "scenario21b: stale prom (1h old) must fail loud (rc=1), got rc=$rc out=$out"
[[ "$out" == "STALE"* ]] || fail "scenario21b: stale prom must print STALE line, got $out"
ok "scenario21b: stale prom (1h old, threshold 1800s) fails loud"
...
[[ "$rc" == "0" ]] || fail "scenario21c: PROD_RATIO_CACHE_FRESHNESS_S must be overridable, got rc=$rc out=$out"
[[ "$out" == "FRESH"* ]] || fail "scenario21c: tighter threshold still passes a fresh prom, got $out"
ok "scenario21c: PRODUCT_RATIO_CACHE_FRESHNESS_S threshold is overridable (600s vs 1800s default)"

set +e
out=$(freshness_drive "$scratch/does-not-exist.prom" 1800); rc=$?
set -e
[[ "$rc" == "1" ]] || fail "scenario21d: missing prom must fail loud (rc=1), got rc=$rc out=$out"
[[ "$out" == "MISSING"* ]] || fail "scenario21d: missing prom must print MISSING line, got $out"
ok "scenario21d: missing prom fails loud (path missing)"
