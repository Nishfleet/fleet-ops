...
# must be classed a COPRSE regardless of HTTP status, instead of resetting to
...
# corpse DUROBLY — over the marker's expired usable_at and over the #3737
...
#   S5  only a comeack_release recovery re-proves the corpse; seat_usable
#       then returns usable.
...
# --- S5: only a comeack_release recovery re-proves the corpse ----------------
...
ok "seat spawn corpse: chronic spawn_fail past the threshold is durably classed a COPRSE (ledger + marker seat_dead=true) and held regardless of HTTP 200, released only by a recovery probe (#3889)"
