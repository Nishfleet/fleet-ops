...
# override. The spawn-fail window stays at its 30 min default.
...
# --- (a) count SURVIVES a gap > 30 min but < 2 h (the live #2934 gap) -----
...
# window and the count reset to 1. With EMPTY_RUN_COUNT_WINDOW_S=7200 the
# 1h42m (6120 s) gap is still inside the 2 h window, so the count must
...
# 30 min spawn-fail window and the 2 h empty-run window.
age_marker "$mf" 6120
...
[[ "$cross_gap_count" == "2" ]] \
    || fail "(a) marker count after a 6120s (1h42m) gap = $cross_gap_count, want 2 — the empty-run count must ACCUMULATE across a gap > 30min but < 2h (fleet-ops#2934); before the fix the 30min window reset it to 1"
ok "(a) empty-run count accumulates 1 -> 2 across a 1h42m gap (> 30min spawn-fail window, < 2h empty-run window) — the live #2934 reset is fixed"
...
[[ "$park_count" == "3" ]] \
    || fail "(b) marker count after 3rd intermittent no-op = $park_count, want 3 — the count must reach the ceiling across < 2h gaps"
...
# past 30 min but inside 2 h, then running an empty-run — the spawn_fail
...
clobber_with_healthy "$p" "$m" "$lf"
# Age past the 30 min spawn-fail window but inside the 2 h empty-run window.
age_marker "$mf" 2400
...
ok "fleet-ops#2934: empty-run count accumulates across gaps up to 2h (intermittent no-op'er reaches the ceiling and is parked), gaps > 2h reset (recovery honoured), and spawn_fail keeps its 30min window"
