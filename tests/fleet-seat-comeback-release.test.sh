...
# --- 20. fleet-ops#3366: seat-caps.json retired-slug guard -----------------
...
# 20a. a corpse ledger for a slug retired as intentional_cap_zero=corpse in
...
EOF
rm -rf "$TMPD/seats20" "$TMPD/seats-corpse-retired-$NOW_ISO"
mkdir -p "$TMPD/seats20"
cat > "$TMPD/seats20/opencode__mimo-v2.5-free.json" << 'EOF'
...
SEAT_CAPS_JSON="$SEAT_CAPS_3366" \
    PI_SEAT_HEALTH_LEDGER_DIR="$TMPD/seats20" \
    FLEET_SEAT_COMEBACK_STATE="$TMPD/state20a.json" \
    FLEET_SEAT_COMEBACK_PROM="$TMPD/prom20a.prom" \
    FLEET_SEAT_COMEBACK_NOW="$NOW_ISO" \
    PI_BIN="$TMPD/pi-tool-ok" \
    bash "$BIN" >/dev/null 2>"$TMPD/ret20a.err"
rc=$?
set -e
[[ "$rc" == "0" ]] || fail "20a: retired-slug corpse sweep must exit 0, got $rc ($(cat "$TMPD/ret20a.err"))"
...
grep -q "skip.*intentionally retired\|force-retiring corpse\|RETIRED opencode/mimo-v2.5-free.*intentionally retired" "$TMPD/ret20a.err" \
  || fail "20a: must log the #3366 retired-slug guard: $(cat "$TMPD/ret20a.err")"
# CRITICAL: no "corpse re-probe" line — the #3156 second-chance re-probe
# must NOT fire for a retired slug.
grep -q "corpse re-probe opencode/mimo-v2.5-free" "$TMPD/ret20a.err" \
  && fail "20a: retired-slug corpse must NOT get a second-chance re-probe (#3366 guard must preempt #3156): $(cat "$TMPD/ret20a.err")" || true
# CRITICAL: no "probing" line — the probe must not even fire.
grep -q "probing opencode/mimo-v2.5-free" "$TMPD/ret20a.err" \
  && fail "20a: retired-slug corpse must NOT be probed at all (#3366 guard): $(cat "$TMPD/ret20a.err")" || true
[[ ! -e "$TMPD/seats20/opencode__mimo-v2.5-free.json" ]] \
  || fail "20a: retired-slug corpse must leave the live roster: $(ls "$TMPD/seats20")"
[[ -f "$TMPD/seats-corpse-retired-$NOW_ISO/opencode__mimo-v2.5-free.json" ]] \
  || fail "20a: retired corpse must land in the dated retirement dir: $(ls -la "$TMPD/seats-corpse-retired-$NOW_ISO" 2>&1)"
grep -q "^fleet_seat_comeback_release_retired_total 1$" "$TMPD/prom20a.prom" \
  || fail "20a: prom retired_total must be 1: $(cat "$TMPD/prom20a.prom")"
grep -q "^fleet_seat_comeback_release_released_total 0$" "$TMPD/prom20a.prom" \
  || fail "20a: prom released_total must be 0 (no re-probe, no unwall): $(cat "$TMPD/prom20a.prom")"
ok "20a: corpse ledger for a seat-caps.json retired slug is force-retired without re-probe (fleet-ops#3366)"

# 20b. a NON-DEAD ledger for a slug retired as intentional_cap_zero=corpse
...
SEAT_CAPS_JSON="$SEAT_CAPS_3366" \
    PI_SEAT_HEALTH_LEDGER_DIR="$TMPD/seats20" \
    FLEET_SEAT_COMEBACK_STATE="$TMPD/state20b.json" \
    FLEET_SEAT_COMEBACK_PROM="$TMPD/prom20b.prom" \
    FLEET_SEAT_COMEBACK_NOW="$NOW_ISO" \
    PI_BIN="$TMPD/pi-tool-ok" \
    bash "$BIN" >/dev/null 2>"$TMPD/ret20b.err"
rc=$?
set -e
[[ "$rc" == "0" ]] || fail "20b: retired-slug non-dead sweep must exit 0, got $rc ($(cat "$TMPD/ret20b.err"))"
grep -q "skip opencode/mimo-v2.5-free.*intentionally retired" "$TMPD/ret20b.err" \
  || fail "20b: must log the #3366 skip for a retired slug: $(cat "$TMPD/ret20b.err")"
grep -q "probing opencode/mimo-v2.5-free" "$TMPD/ret20b.err" \
  && fail "20b: retired-slug non-dead seat must NOT be probed (#3366 guard): $(cat "$TMPD/ret20b.err")" || true
# The ledger must be UNTOUCHED (no re-bench, no unwall, no corpse).
health=$(jq -r '.health_class' "$TMPD/seats20/opencode__mimo-v2.5-free.json")
[[ "$health" == "rate_limited" ]] \
  || fail "20b: retired-slug non-dead ledger must be untouched (health_class still rate_limited, got $health)"
count=$(jq -r '.consecutive_failure_count' "$TMPD/seats20/opencode__mimo-v2.5-free.json")
[[ "$count" == "3" ]] \
  || fail "20b: retired-slug non-dead ledger count must be untouched (still 3, got $count)"
grep -q "^fleet_seat_comeback_release_released_total 0$" "$TMPD/prom20b.prom" \
  || fail "20b: prom released_total must be 0: $(cat "$TMPD/prom20b.prom")"
grep -q "^fleet_seat_comeback_release_retired_total 0$" "$TMPD/prom20b.prom" \
  || fail "20b: prom retired_total must be 0: $(cat "$TMPD/prom20b.prom")"
ok "20b: non-dead ledger for a seat-caps.json retired slug is skipped silently (no probe, no re-bench, fleet-ops#3366)"

# 20c. dry-run: a retired-slug corpse is previewed for force-retirement, the
...
out=$(SEAT_CAPS_JSON="$SEAT_CAPS_3366" \
    PI_SEAT_HEALTH_LEDGER_DIR="$TMPD/seats20" \
    FLEET_SEAT_COMEBACK_STATE="$TMPD/state20c.json" \
    FLEET_SEAT_COMEBACK_PROM="$TMPD/prom20c.prom" \
    FLEET_SEAT_COMEBACK_NOW="$NOW_ISO" \
    PI_BIN="$TMPD/pi-tool-ok" \
    bash "$BIN" --dry-run 2>&1)
grep -q "DRY-RUN: would retire opencode/mimo-v2.5-free.*intentionally retired" <<<"$out" \
  || fail "20c: dry-run must preview force-retirement of the retired-slug corpse: $out"
grep -q "corpse re-probe opencode/mimo-v2.5-free" <<<"$out" \
  && fail "20c: dry-run must NOT preview a re-probe for a retired slug: $out" || true
# Dry-run must not touch the ledger.
[[ -f "$TMPD/seats20/opencode__mimo-v2.5-free.json" ]] \
  || fail "20c: dry-run must not move the corpse ledger"
[[ ! -d "$TMPD/seats-corpse-retired-$NOW_ISO" ]] \
  || fail "20c: dry-run must not create a retirement dir"
ok "20c: dry-run previews force-retirement of a retired-slug corpse without acting (fleet-ops#3366)"

# 20d. a non-retired slug in the same seat-caps.json is NOT affected by the
...
rm -rf "$TMPD/seats20" "$TMPD/seats-corpse-retired-$NOW_ISO"
mkdir -p "$TMPD/seats20"
cat > "$TMPD/seats20/opencode__nemotron-3-ultra-free.json" << 'EOF'
{"provider":"opencode","model":"nemotron-3-ultra-free","http_status":null,"retry_after":null,"health_class":"corpse","retryable":false,"seat_dead":true,"poison_ladder":false,"observed_at":"2026-08-30T11:00:00Z","source":"comeback_release_corpse","failure_mode":"comeback_never_released","usable_at":null,"bench_until":null,"consecutive_failure_count":25}
EOF
set +e
SEAT_CAPS_JSON="$SEAT_CAPS_3366" \
    PI_SEAT_HEALTH_LEDGER_DIR="$TMPD/seats20" \
    FLEET_SEAT_COMEBACK_STATE="$TMPD/state20d.json" \
    FLEET_SEAT_COMEBACK_PROM="$TMPD/prom20d.prom" \
    FLEET_SEAT_COMEBACK_NOW="$NOW_ISO" \
    PI_BIN="$TMPD/pi-tool-ok" \
    bash "$BIN" >/dev/null 2>"$TMPD/ret20d.err"
rc=$?
set -e
[[ "$rc" == "0" ]] || fail "20d: non-retired slug sweep must exit 0, got $rc ($(cat "$TMPD/ret20d.err"))"
...
grep -q "corpse re-probe opencode/nemotron-3-ultra-free: no wall, second-chance re-probe" "$TMPD/ret20d.err" \
  || fail "20d: non-retired slug must get the normal #3156 re-probe (guard must NOT fire): $(cat "$TMPD/ret20d.err")"
grep -q "UNWALLED opencode/nemotron-3-ultra-free" "$TMPD/ret20d.err" \
  || fail "20d: non-retired slug must be unwalled by the healthy re-probe: $(cat "$TMPD/ret20d.err")"
grep -q "^fleet_seat_comeback_release_released_total 1$" "$TMPD/prom20d.prom" \
  || fail "20d: prom released_total must be 1: $(cat "$TMPD/prom20d.prom")"
ok "20d: non-retired slug in the same seat-caps.json gets the normal #3156 re-probe (guard is scoped to intentional_cap_zero=corpse only, fleet-ops#3366)"

echo "ALL OK: active come-back release path (fleet-ops#2421) + force-probe-on-overdue-usable_at + corpse-at-threshold + never-released metric (fleet-ops#2638) + own-streak corpse + interval-breach loud check (fleet-ops#2806) + no-wall corpse second-chance re-probe / explicit retire (fleet-ops#3156) + extension-reclassify race (fleet-ops#3179) + PQE 1h==1h deadlock fix (fleet-ops#3176) + skip-corpse-on-reanchored-wall (fleet-ops#3301) + seat-caps.json retired-slug guard (fleet-ops#3366)"
