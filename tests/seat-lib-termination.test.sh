...
# EXCEPT lib/litellm-seat.sh itself, must be:
#   (a) defined by the calling file, or
#   (b) defined by lib/litellm-seat.sh, which the calling file sources.
#
# (lib/litellm-seat.sh is exempt from its own scan: the #6101 packet forbids
# touching it, and its remaining mentions of 198-names are locals, $vars and
# jq '.field' reads, not calls — verified 2026-09-13 while writing this test.
# A separate #4263-family packet owns that file.)
#
...
# #5993-pick-seat-freeze style, instead of being hunted into false 127s.
...
# Collect the #!-sh bins plus lib/*.sh, minus the exempted definer module.
targets=()
...
for f in "${targets[@]}"; do
    ...
    bodies="$(sed -e 's/#.*//' "$f" 2>/dev/null | grep -vE '^[[:space:]]*(local|declare|typeset|readonly)[[:space:]]')"
    [ -n "$bodies" ] || continue
    hits="$(printf '%s\n' "$bodies" | grep -oE "(^|[^a-zA-Z0-9_\"'(.]|\$\()(${names_regex})([[:space:];)|]|$)" || true)"
    [ -n "$hits" ] || continue
    for name in $(printf '%s\n' "$hits" | sed -E "s/^[^a-zA-Z_]+|[[:space:];)|)]+$//g" | sort -u); do
        # A definition inside the calling file satisfies the clause.
        printf '%s\n' "$bodies" | grep -qE "(^|[[:space:]])${name}[[:space:]]*\\(\\)" && continue
        # Otherwise the calling file must source the one surviving definer.
        grep -q 'litellm-seat\.sh' "$f" && continue
        # Else it must be a recorded prose mention, not a 127 call.
        grep -qxF "$f:$name" <<<"$prose_mentions" && continue
        echo "VIOLATION: $f calls $name but neither defines it nor sources lib/litellm-seat.sh"
