...
import { isLocalRicher } from "./.github/scripts/repo-standards.lib.mjs";
const assert = (cond, msg) => { if (!cond) { console.error("FAIL " + msg); process.exit(1); } else console.error("ok " + msg); };
// A plain (non-LOCAL_RICHER) repo gets the thin-caller required contexts —
// its secret-scan/semgrep thin callers produce exactly these.
assert(!isLocalRicher("Nishfleet/TinyStudio.io"), "TinyStudio.io is not LOCAL_RICHER (fixture premise)");
const std = mergeQueueRequiredContexts("infra", [], [], "Nishfleet/TinyStudio.io");
assert(JSON.stringify(std) === JSON.stringify(["Gitleaks", "semgrep"]), `plain infra repo with no extras -> standard thin-caller contexts (got ${JSON.stringify(std)})`);
// standard+BP union (dedup, order-stable)
const union = mergeQueueRequiredContexts("infra", ["gate-integrity", "P14 tests"], ["arm"], "Nishfleet/TinyStudio.io");
...
const union2 = mergeQueueRequiredContexts("infra", ["kept-check", "Gitleaks"], ["kept-check"], "Nishfleet/TinyStudio.io");
...
// LOCAL_RICHER: fleet-ops produces its own contexts (Gitleaks+Semgrep folded
// into the "P14 tests" job) — the issue pins its required contexts to EXACTLY
// what its branch protection already carries. Adding the thin-caller contexts
// there would require checks nothing runs: a 6h-timeout deadlock on every
// merge-queue group (fleet-ops#5787).
const fleetOpsCtx = ["arm", "CI checklist gate", "gate-integrity", "P14 tests"];
assert(isLocalRicher("Nishfleet/fleet-ops"), "fleet-ops is LOCAL_RICHER");
const fleetOps = mergeQueueRequiredContexts("infra", fleetOpsCtx, [], "Nishfleet/fleet-ops");
assert(JSON.stringify(fleetOps) === JSON.stringify(fleetOpsCtx), `fleet-ops (LOCAL_RICHER) requires exactly its own 4 contexts, no phantom Gitleaks/semgrep (got ${JSON.stringify(fleetOps)})`);
// 0509: same — its own 6 contexts, nothing added, nothing dropped.
const c0509 = ["Gitleaks", "codex-node-checks", "required-verifier-integrity", "semgrep", "preview-assert", "release-proof"];
const z0509 = mergeQueueRequiredContexts("infra", c0509, c0509, "Nishfleet/0509");
assert(JSON.stringify(z0509) === JSON.stringify(c0509), `0509 (LOCAL_RICHER) keeps exactly its 6 contexts (got ${JSON.stringify(z0509)})`);
JS
