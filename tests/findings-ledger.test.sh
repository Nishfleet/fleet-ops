...
# Hermetic: every artifact lives in a private mktemp dir; no fixed /tmp path
# survives (issue #6102 — 5 parallel copies collided on the shared ledger).
# FINDINGS_LEDGER_TEST still overrides the ledger path for drillers; no gh
# calls when gh_issue_map is None (no-issues backfill mode).
set -uo pipefail
HERE="$(cd "$(dirname "$0")/.." && pwd)"
PY="$HERE/lib/findings_ledger.py"
TD="$(mktemp -d)"
trap 'rm -rf "$TD"' EXIT INT TERM
LEDGER="${FINDINGS_LEDGER_TEST:-$TD/findings-ledger.jsonl}"
rm -f "$LEDGER"
fail=0
...
# 1. happy path
before=$(wc -l 2>/dev/null < "$LEDGER" || echo 0)
LA --source-organ test --run-id r1 --title "Overflow queue" --severity high \
   --evidence-ref "$TD/e.json" --disposition filed --ref "fleet-ops#5442" --reason "test"
new=$(wc -l < "$LEDGER")
...
D="$TD/reports"; rm -rf "$D"; mkdir -p "$D/20260901T000000Z"
...
python3 "$PY" import-md --ledger "$TD/imp.jsonl" --source-organ outside-in-audit --file "$P" >/dev/null
grep -q '"disposition": "filed"' "$TD/imp.jsonl" && grep -q "0509#2962" "$TD/imp.jsonl"
t $?

# 11. corrupt JSONL row refuses validate
printf 'not-json\n' >> "$TD/bad.jsonl"
python3 "$PY" validate --ledger "$TD/bad.jsonl" >/dev/null 2>&1
[ "$?" -ne 0 ]; t $?
