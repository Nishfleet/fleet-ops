...
case "$1" in
  api)
    ...
    # After the gh-json-graphql migration (auditor 2026-08-27T19:00Z
    # follow-up), claim-reconcile uses REST for the open-PR probe and
    # the issue state load. The call shapes are:
    #   gh api repos/<owner>/<repo>/pulls?state=open&head=<owner>:claim/issue-<N>&per_page=1
    #   gh api repos/<owner>/<repo>/issues/<N>
    # Map the PR probe to the per-issue fixture file the pr-list branch
    # would have used; fall through to the file-backed lookup for the
    # issue-state GET.
    if [[ "$path" =~ ^repos/([^/]+)/([^/]+)/pulls\?.*head=([^:&]+):claim/issue-([0-9]+) ]]; then
      owner="${BASH_REMATCH[1]}"; reponame="${BASH_REMATCH[2]}"
      n="${BASH_REMATCH[4]}"
      repo="${owner}/${reponame}"
    ...
    # Issue state load: REST returns the same shape the old `gh issue
    # view --json` did (lowercase state, labels[].name), so the fixture
    # written by `write_issue` is directly reusable here. Fall back to
    # the per-issue fixture under $FAKE_DIR/issues/ first; only if that's
    # absent do we use $FAKE_DIR/api/<path>.json for test scenarios that
    # intentionally mock a different REST payload (e.g. 404).
    if [[ "$path" =~ ^repos/[^/]+/[^/]+/issues/([0-9]+) ]]; then
      num="${BASH_REMATCH[1]}"
      for d in issues prs api; do
        f="$FAKE_DIR/$d/${path#repos/}.json"
        # $path has no query string for issue lookups, so this matches
        # both $FAKE_DIR/issues/Nishfleet/fleet-ops/<N>.json and any
        # $FAKE_DIR/api/.../<N>.json the test may have staged.
