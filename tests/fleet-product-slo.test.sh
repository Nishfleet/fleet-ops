...
#   (p) fleet-ops#5001: the 24h outcome windows are TRUE trailing windows. A
...
# =========================================================================
# (p) fleet-ops#5001 boundary: the trailing 24h window must be a TRAILING
...
def fake_urlopen(req, timeout=None):
    ...
    n = conn.execute(run_sql).fetchone()[0]
    payload = json.dumps(
        {"success": True, "result": [{"results": [{"n": n}]}]}
    ).encode("utf-8")
...
assert len(captured) == 4, f"expected 4 D1 queries, captured {len(captured)}"
...
PY
ok "(p) 24h boundary: 25h-ago row excluded from signups/activated/briefs, 23h counted"
