...
#
# fleet-ops#4181 P2 + #4263 P3b: proves agent-cron-run fable-check routes to
# the LiteLLM proxy (litellm/judge) and every other slug routes through
# litellm_seat "worker-cheap". The proxy owns the fallback chain; the
# seatlib pick path is deleted.
#
# Acceptance:
#   - fable-check slug -> pi invoked with --provider litellm --model judge
#   - litellm_seat is NOT called for fable-check (judge short-circuit)
#   - a non-fable-check slug uses litellm_seat worker-cheap
#   - AGENT_CRON_SKIP_LITELLM=1 drops fable-check to the worker-cheap group
#     (the seatlib escape hatch is gone — the env now only skips judge)
#
# Offline. Stubbed litellm-seat lib + stubbed pi. No live proxy needed.
...
# Stub seat lib: litellm_seat records that it was called and returns a seat.
# The test checks litellm_seat was NOT called for fable-check.
...
seat_record="$scratch/seat-pick.log"
...
export SEAT_PICK_RECORD="$seat_record"
export HERMES_RECORD="$scratch/hermes.log"
export ATTEMPTS_DIR="$scratch/attempts"

# --- scenario 1: fable-check -> litellm/judge, litellm_seat NOT called -------
rm -f "$record_args" "$record_stdin" "$seat_record"
set +e
...
[[ ! -f "$seat_record" ]] \
  || fail "scenario 1: litellm_seat must NOT be called for fable-check, got: $(cat "$seat_record")"
ok "scenario 1: fable-check -> litellm/judge, litellm_seat bypassed"

# --- scenario 2: non-fable slug uses litellm_seat worker-cheap ---------------
rm -f "$record_args" "$record_stdin" "$seat_record"
set +e
...
grep -q -- '--provider litellm' "$record_args" \
  || fail "scenario 2: pi must run on provider litellm (from litellm_seat), got: $(cat "$record_args")"
grep -q -- '--model worker-cheap' "$record_args" \
  || fail "scenario 2: pi must run on model worker-cheap (from litellm_seat), got: $(cat "$record_args")"
grep -q 'litellm_seat CALLED worker-cheap' "$seat_record" \
  || fail "scenario 2: litellm_seat worker-cheap must be called for non-fable slugs"
ok "scenario 2: non-fable slug -> litellm_seat worker-cheap"

# --- scenario 3: AGENT_CRON_SKIP_LITELLM=1 -> worker-cheap group -------------
rm -f "$record_args" "$record_stdin" "$seat_record"
set +e
...
grep -q -- '--provider litellm' "$record_args" \
  || fail "scenario 3: pi must run on provider litellm (skip-litellm degrades to worker-cheap), got: $(cat "$record_args")"
grep -q -- '--model worker-cheap' "$record_args" \
  || fail "scenario 3: pi must run on model worker-cheap when AGENT_CRON_SKIP_LITELLM=1, got: $(cat "$record_args")"
ok "scenario 3: AGENT_CRON_SKIP_LITELLM=1 -> worker-cheap group (seatlib hatch deleted)"

ok "fable-check-litellm-routing: fable-check routes to litellm/judge, non-fable uses worker-cheap, skip hatch degrades to worker-cheap"
||||||| (empty tree)
=======
#!/usr/bin/env bash
# tests/agent-cron-fable-check-litellm-routing.test.sh
