...
litellm_group_for_privacy() {
  # Mirror of the real pi-packet/litellm-seat.sh mapping (fleet-ops#520:
  # the resume rail fails closed to the private group).
  if [[ "${1:-public}" == "private" ]]; then
    echo "worker-private"
  else
    echo "worker-cheap"
  fi
...
# The #1354/#2661 cases above leave SEAT_MODE=flash; the resume rail needs a
# healthy worker seat, so reset it here (a sticky export would otherwise
# silently starve the relaunch — exactly the silent-gap class this issue
# closes).
export STOP_ESCALATION_TEST_SEAT_MODE=healthy
...
grep -q -- '--label agent-ready' "$STOP_ESCALATION_TEST_GH_LOG" || fail "resume dispatch: issue must carry the agent-ready label or intake never picks it up: $(cat "$STOP_ESCALATION_TEST_GH_LOG")"
...
# The drill's live names are live-dummy-resume-drill-* so the INSTALLED
# writer's live-dummy* exclusion keeps a pre-merge drill hermetic; the
# dispatcher's infix sink must still catch them.
: > "$AS/resume-drill/issues.jsonl"; : > "$STOP_ESCALATION_TEST_GH_LOG"
resume_sr "live-dummy-resume-drill-nodeliv-t0.service" "unit-stopped-without-deliverable"
set +e; "$dispatch"; rc=$?; set -e
[[ $rc -eq 0 ]] || fail "drill sink (live-dummy name): expected exit 0, got $rc"
[[ -s "$STOP_ESCALATION_TEST_GH_LOG" ]] && fail "drill sink (live-dummy name): must never touch GitHub"
