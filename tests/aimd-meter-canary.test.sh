...
! grep -q 'issue create' "$gh_log" || fail "scenario3: must not file when the reader is back"
...
! grep -q 'issue create' "$gh_log" || fail "scenario4: must not create a second issue"
