...
run_tier1() { # <extra env assignments...> -- runs tier1 with scratch HOME
    env -i \
        ...
        "$@" \
        timeout "${TIER1_TIMEOUT:-20}" bash "$tier1"
}
...
: >"$scratch/gh.log"
proceed_out="$(run_tier1 GH_APP_RESERVE_FLOOR=100 2>&1 || true)"
if grep -q "reserve floor reached" <<<"$proceed_out"; then
    fail "floor=100 with remaining=300 must NOT hold; out: $proceed_out"
fi
[[ -s "$scratch/gh.log" ]] || fail "above the floor the tick must proceed into its sections (gh calls expected); tick output: $proceed_out"
