...
# fleet-ops#4640: the weekly false-wall drill retired WITH its organ
# (fleet-ops#6101 deleted fleet-seat-comeback-release; the #4640 sub-step
# went with it — the prompt must not promise a drill that cannot run).
grep -q -- '--false-wall-drill' "$prompt" \
  && fail "prompt still promises the retired #4640 false-wall drill (fleet-ops#6101)"
ok "(f) prompt locks the 5-action cap, signal, blind 8-lens structure (incl. SECURITY), claimed-work-only, baseline-delta input, #4471 CONFLICTING namecheck; #4640 drill retired (fleet-ops#6101)"
