...
# the first match.
#
# GitHub's search-index delay (fleet-ops#951) filed 6 copies of
#   signal: failed-command-flagged/2026-08-26t13-18-31-426z-01a03e38-e602-737c-b399-576dcf48d08e
# at 2026-08-27T05:11-05:16Z. Leftovers still open at filing time of
# #966: #952, #957, #966, #971, #976, #981. The python-traceback shape
# (live #957) is the class these duplicates share — `python3 -c` /
# `python3 << 'EOF'` probe crashing with a KeyError or NameError +
# "Command exited with code 1", walked past with no user-facing flag.
...
# a CAP that silently drops the rest forever, would leave #966 (and its
# siblings) dispatching workers after the session has aged out.
#
# The python-traceback shape itself is locked under #957
# (tests/fleet-failed-command-python-traceback.test.sh). The leftover-
# duplicate drain class is locked under #965 for the edit-unmatch pile
# (tests/fleet-failed-command-observe-duplicate-open.test.sh). This
...
# Live leftover open duplicates (as of 2026-08-27T09:33Z): #952, #957, #966, #971, #976, #981
#
# Scenarios:
#   1. green tick, six leftover open issues + one unrelated issue
#      with no failed-command signal: comments resolved-at on all
#      six leftovers, touches none of the unrelated issue, does not
#      close same tick.
...
scratch="$(mktemp -d -t failed-command-observe-dup-pytrace.XXXXXX)"
...
# Live leftover pile from the 01a03e38 python-traceback session.
# These six issues are the live leftover open duplicates of #957
# (#952, #957, #966, #971, #976, #981). A refactor that resolves only
# issue 0 of this pile is a regression; this test must catch it.
slug="2026-08-26t13-18-31-426z-01a03e38-e602-737c-b399-576dcf48d08e"
leftovers=(952 957 966 971 976 981)
unrelated=999
...
seed_leftovers
# No session file for the 01a03e38 slug: the 24h window has aged it
...
ok "live #966: green tick comments resolved-at on all six leftover duplicates, not first-only"
...
ok "live #966: later tick closes all six leftover duplicates"
...
{"type":"message","message":{"role":"assistant","content":[{"type":"toolCall","id":"call_retry1","name":"bash","arguments":{"command":"python3 -c \"import json; v1=json.load(open('/home/nish/workspaces/agent-state/0509-transformation/discovery-spike/results.json')); print(list(v1[0].keys()))\""}}]}}
JSONL
