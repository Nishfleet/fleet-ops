...
# restores $real_home.
# $HOME here may ALREADY be a caller's redirected scratch (the coverage
# canary exports HOME="$scratch/home", fleet-ops#5456) — the unit search
# path belongs to the systemd --user manager, which runs as THIS uid, so the
# manager's home is this uid's passwd entry, not the inherited $HOME.
real_home="$(getent passwd "$(id -un 2>/dev/null)" 2>/dev/null | cut -d: -f6)"
