...
- `lib/pi-intake-tick.sh`: when the WORK cap fires, intake forces the next claim onto a DIFFERENT seat class via a per-issue `.prefer-class` ladder (prepaid -> metered -> senior), resetting reclaim-count to 1 so the new class gets a fresh budget. Only at ladder exhaustion (senior already tried) does intake block — and then with `blocked-on: infra` (the senior conference is the orchestrator's job, and a conference that never runs is not an unblocking path), not the old `blocked-on: nish-decision`. Infra deaths never reach the WORK cap at all, so a provider storm can no longer park the issue.
- `bin/blocked-reconcile`: fleet-ops#3527 scope merged into #3310. `blocked-on: infra` is now a technical, auto-releasable blocker. Aged >= 2h with a healthy capable seat reported by `seat-lib`/`pick_seat`, it resets the issue's WORK cap (`reclaim-count`) and `.systemic` markers, re-queues as `agent-ready`, and records `blocked-reconcile: infra-block released`. A second such release within 24h escalates the issue to `blocked-on: senior-review` instead of another requeue. `blocked-on: senior-review` stays blocked until the senior-review path resolves it.
...
- Prevention mechanism (mechanical-fix rule): `tests/fleet-ops-3310-infra-death-class-switch.test.sh` is a 16-test gate, Tests 10-15 being a replay drill that RUNS the real `pi-issue-run` (fake pi binary: hang rc=124, ordinary rc=1) and the real `pi-issue-failed-reap` (stubbed gh/systemctl) against scratch dirs and asserts the counter split end-to-end. `tests/blocked-reconcile.test.sh` now includes live-sweep fixtures proving the `blocked-on: infra` auto-release path, the 2h minimum-age gate, the healthy-capable-seat check, the second-release-within-24h `blocked-on: senior-review` escalation, and the senior-review stay-blocked path. Hosted from `tests/ci-standards-audit.test.sh` (the P14 suite) + pinned in `tests/p14-test-listing-gate.test.sh`.
...
| `bash tests/blocked-reconcile.test.sh` | `ALL OK` including fleet-ops#3310/#3527 infra auto-release and senior-review escalation fixtures, exit 0 |
...
run-proof: replay drill inside `tests/fleet-ops-3310-infra-death-class-switch.test.sh`: Test 10 runs the real `pi-issue-run` with a fake pi that hang-dies (rc=124) and asserts `.last-death-class=infra`; Test 12 runs the real `pi-issue-failed-reap` against a stubbed gh/systemctl and asserts `.infra-death=1` with `.reclaim-count` absent — the provider-storm case that previously parked fleet-ops#3310's five packets now leaves the WORK cap untouched. The live-sweep fixtures in `tests/blocked-reconcile.test.sh` run the real `bin/blocked-reconcile` with a mocked `gh` and fake `seat-lib` state, asserting that an aged `blocked-on: infra` issue with a healthy capable seat is re-queued and its WORK cap reset, a second release within 24h posts `blocked-on: senior-review`, and `blocked-on: senior-review` stays blocked.
