...
#                             exits 0 when fresh state shows remaining below
#                             the reserve floor, so sweeps/organs pause while
#                             intake claims keep their floor.

# Paths are env-overridable so tests run offline against scratch dirs.
GH_BUDGET_STATE="${GH_BUDGET_STATE:-/home/nish/workspaces/agent-state/pi-intake/gh-rate-limit.json}"
GH_BUDGET_TRIAGE="${GH_BUDGET_TRIAGE:-${FLEET_HEARTBEAT_TRIAGE:-/home/nish/workspaces/agent-state/FLEET-HEARTBEAT-TRIAGE.md}}"
GH_BUDGET_LAST_LOUD="${GH_BUDGET_LAST_LOUD:-/home/nish/workspaces/agent-state/pi-intake/gh-budget-last-loud}"
...
gh_budget_loud() {
    ...
    if [[ -r "$GH_BUDGET_LAST_LOUD" ]]; then
        read -r last <"$GH_BUDGET_LAST_LOUD" 2>/dev/null || last=0
