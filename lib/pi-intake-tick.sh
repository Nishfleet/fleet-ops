...
blocked-on: infra" 2>/dev/null || true
    ...
    if [[ -n "$_claims_log_snapshot" ]]; then
        ...
        if (( _cl_window_claims >= MAX_CLAIMS_IN_WINDOW )); then
            ...
            # 2026-09-10). Match both: a `decision-resolved:` token anywhere
            # in a comment that does NOT itself carry a live blocked-on: line
            # (instruction comments carry a copy-paste example, not an answer).
