...
blocked-on: orchestrator" 2>/dev/null || true
    ...
    # fleet-ops#5048: delegated-range park — the cell the two park branches
    # above miss. A PROTECTED open issue past PARK_MAX_CLAIMS with NO merged
    # claim/issue-<N> PR and no `termination:`-gated delivery, whose remaining
    # work is delegated to a named runtime event: an explicit
    # `awaiting: <name>` / `blocked-on: timer <name>` marker, or a per-issue
    # unit (its name carries this issue number) that is LIVE under
    # `systemctl --user`. live: #4891 — delivered by fable/* PRs #4893/#4897
    # and delegated to `remeasure-4891-timer` (fires 2026-09-11T14:05Z); it
    # carries neither a `termination:` clause nor a merged claim-branch PR,
    # so the #4540 park missed it and it re-entered the claimable pool on
    # every release: 12 claims in one day, each burning a seat pick plus a
    # worker run re-verifying a date-gate that could not be met yet.
    #
    # PROTECTED-ONLY by design: on a claimable non-protected issue a bare
    # mention is not a fix (fleet-ops#3231), so a weak signal must never park
    # one. Nothing here is inferred from prose — the evidence is a declared
    # marker or a real live unit. Runs after the shared comments fetch and
    # reuses `comments`, so the only added probe is the merged-PR list, paid
    # by a protected over-cap issue only — and it sits before blocked_filter
    # so a `blocked-on: timer` marker escalates to a real park instead of a
    # per-tick skipped-blocked-on that re-verifies forever.
    if (( _park_claims > PARK_MAX_CLAIMS )) && [[ "${_park_protected:-0}" == 1 ]]; then
        ...
        if ! printf '%s' "$_p48_merged" | jq -e 'length > 0' >/dev/null 2>&1; then
            ...
            if printf '%s' "$_p48_text" | grep -qE '^(awaiting:[[:space:]]*[^[:space:]]+|blocked-on:[[:space:]]*timer[[:space:]]+[^[:space:]])'; then
                ...
                # Text may name the base (`remeasure-4891-timer`) or the unit
                # (`remeasure-4891-timer.timer`); probe every plausible form.
                while IFS= read -r _p48_cand; do
                    [ -z "$_p48_cand" ] && continue
                    # This issue's own claim worker (pi-issue@<repo>-<N> or
                    # pi-issue-<repo>-<N>) in flight is a claim, never a
                    # delegation.
                    case "$_p48_cand" in pi-issue@*|pi-issue-*) continue ;; esac
                    # Per-issue unit only — the issue number must appear with
                    # non-digit boundaries, so #489 never matches a unit named
                    # for #4891 (nor #4891x for #4891).
                    printf '%s' "$_p48_cand" | grep -qE "(^|[^0-9])${N}([^0-9]|\$)" || continue
                    case "$_p48_cand" in
                        *.timer|*.service) _p48_forms=("$_p48_cand") ;;
                        *)                 _p48_forms=("$_p48_cand.timer" "$_p48_cand.service" "$_p48_cand") ;;
                    esac
                    for _p48_try in "${_p48_forms[@]}"; do
                        if "$SYSTEMCTL" --user is-active --quiet "$_p48_try" 2>/dev/null; then
                            _p48_evidence="delegated to live runtime unit $_p48_try"
                            break
                        fi
                    done
                    [ -n "$_p48_evidence" ] && break
                done < <(printf '%s' "$_p48_text" | grep -oE '[A-Za-z0-9_.@-]+[.-](timer|service)\b' | sort -u)
            fi
