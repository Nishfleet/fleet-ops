    ...
    if [[ $comment_rc -ne 0 ]]; then
        case "$comment_out" in
            *"submitted too quickly"*|*"secondary rate"*|*"429"*)
                # Transient GitHub secondary rate limit after retry exhaustion:
                # the claim is already valid (claim branch pushed + label
                # flipped above). The comment is a non-essential "claimed by"
                # breadcrumb, not part of the claim contract. Log and continue
                # to packet+spawn so a sustained addComment rate limit cannot
                # stall the tick or orphan the claim (restart-storm feedback
                # loop, fleet-ops intake 2026-08-29). A permanent error below
                # still fails loud.
                echo "issue $N ($title): comment skipped (secondary rate limit after retries): $comment_out" >&2
                ;;
            *)
