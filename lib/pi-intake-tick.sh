...
for i in "${!numbers[@]}"; do
    ...
    # are legit work (repair/upgrade by titled classification) fall through
    # to the body fetch + allow_claim path so the fleet does not idle when
    # all surge_leverage issues are already claimed/in-progress. Title-only
    # classification here (no body fetch yet) — prefixed titles are
    # authoritative; unprefixed titles without a repair signal in the title
    # stay skipped to preserve the body-fetch avoidance. Churn is always
    # skipped.
