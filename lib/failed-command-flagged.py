...
(fleet-ops#956, #965, #1079, #1218) — or the variant
...
`write` of the same path, is not a user-facing flag. fleet-ops#1218 is a fresh
occurrence of the exact-text class on a DIFFERENT session slug (the
01a043c8 keystone-routing session, where the worker edited
`/tmp/fleet-ops/tests/keystone-routing.test.sh` in a temp clone, got
the live exact-text message, and walked past it with thinking that
named the CAUSE plus action-prose "Let me just rewrite the entire
test file from scratch" plus a successful `write` of the same path).
Action-prose that names the next step is the same swallowed-failure
class as the #1059 "The file was archived" prose and the #1139
"The text is already the same" prose: the user is told what will
happen next, never that the edit failed. A `/tmp/` clone path is not
a probe and a successful rewrite does not discharge the unmatch.
For the #1139 no-op shape the live recovery was
...
write-recovery (#1218), multi-match, and no-op shapes; tests/fleet-failed-command-edit-array-
...
# text" (fleet-ops#956, #965, #970, #975, #980, #1079, #1218), "Found N occurrences"
...
# successful `write` of the same path (#1218), and
...
def result_failed(
    ...
    # / #965 / #1079 / #1218), "Found N occurrences" (many matches, fleet-ops#1053), and
    ...
    # worker believed the file changed and it did not. #1218's write
    # recovery is the other tempting one — the file ended up rewritten —
    # but a successful `write` of the same path does not name the failure.
