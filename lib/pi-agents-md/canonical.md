...
| Model/lane routing — `pi --print --provider <provider> --model <model>` direct; old `~/workspaces/tooling/nish-vault/_system/shared-memory/codex-model-routing.md` ladder is history only | `~/workspaces/agent-state/lanes/pi-seat-health.json` |
