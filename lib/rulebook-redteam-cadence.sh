...
# (fleet-ops#527).
#
# Sourced by bin/fleet-heartbeat-tier1 §22. Also runnable:
...
heartbeat_rulebook_redteam_cadence_canary() {
    ...
    if [ "${RULEBOOK_CADENCE_DISABLE:-0}" = "1" ]; then
        ...
        _rulebook_log "22. rulebook red-team cadence canary DISABLED (RULEBOOK_CADENCE_DISABLE=1)"
        return 0
    fi
    ...
    if [ -z "$plan" ] || [ ! -f "$plan" ]; then
        ...
        _rulebook_log "22. rulebook red-team cadence canary: status=$rulebook_canary_status age=${rulebook_canary_age_s}s limit=${max_age_s}s"
        return 0
    fi
...
heartbeat_rulebook_redteam_heading_bonus() {
    ...
    if [ "${RULEBOOK_HEADING_BONUS_DISABLE:-0}" = "1" ]; then
        ...
        _rulebook_log "22. heading-growth bonus DISABLED"
        return 0
    fi
    ...
    if [ ! -f "$rules" ]; then
        ...
        _rulebook_log "22. heading-growth bonus: standing-rules missing at $rules"
        return 0
    fi
    ...
    if [ "$now_n" -le "$last_n" ]; then
        ...
        _rulebook_log "22. heading-growth bonus: headings=$now_n last=$last_n — no start"
        return 0
    fi
    local unit_state
    ...
    case "$unit_state" in
        active|activating)
            ...
            _rulebook_log "22. heading-growth bonus: $unit is $unit_state — no-op"
            return 0
            ;;
    esac
    if "$systemctl_bin" --user start --no-block "$unit" >/dev/null 2>&1; then
        rulebook_heading_bonus="started"
        _rulebook_log "22. heading-growth bonus: started $unit (headings $last_n -> $now_n)"
    else
