...
try {
  ...
  try {
    ...
    if (suppressedFindings > 0) {
      // Silent-drop sweep 2026-09-11: suppressed cap-overflow findings must be
      // counted in the report body itself (the scout judge reads this), never
      // silently discarded.
      process.stdout.write(`LOUD: ${suppressedFindings} additional findings suppressed by MAX_FINDINGS=${MAX_FINDINGS} — raise the cap or read scout-money-path evidence\n\n`);
    }
  } finally {
    await browser.close();
  }
