...
(
    ...
    # Read once, then the fd is closed: rm-before-iterate kills the SC2094
    # read-while-write coupling, and the pass count comes from the hoisted
    # total (no second <redirect after the file is gone).
    # shellcheck disable=SC2030  # the export targets exactly the child suites
    export MEASURE_P14_MAIN=0
    local suites=0 total
    total=$(wc -l < "$list")
    local -a suite_names=()
    mapfile -t suite_names < "$list"
    rm -f "$list" 2>/dev/null || true
    for name in "${suite_names[@]}"; do
        [ -n "$name" ] || continue
        if [ "$(date +%s)" -ge $((t0 + budget)) ]; then
            printf 'p14-main(main=%s): UNAVAILABLE:timeout(after=%d/%d suites) detail=%s\n' \
                "${main_sha:0:12}" "$suites" "$total" "$detail"
            return 0
        fi
