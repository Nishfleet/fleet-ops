...
# auditor. fleet-ops#3125: product picks (PI_PICK_ROLE=product, exported by
# pi-issue-run and pi-packet-run) order every candidate seat by the rolling
# last-20-sessions PR yield in seat-yield.json descending — ties break by
# class prepaid-quota -> metered -> free so prepaid still drains first among
# equal performers; seats with <20 measured sessions get provisional yield
# 0.5. Unmarked packets keep the class ladder: free first, then prepaid,
# then metered.
#
# fleet-ops#1167: cursor is keystone/senior-review only.
# Every pick appends seat-selection.jsonl and refreshes
# fleet_seat_selection_24h{provider=} for the digest / Weekly Review.
#
# AIMD learned caps (fleet-ops#217, re-land #424; model granularity #3125):
# the declared cap is the FLOOR. pick_seat may admit cap+1 on a lane with
# room below max_probe_ceiling, zero 429s, and RAM headroom — at provider
# level and, for model rows carrying max_probe_ceiling (devin glm-5-2 /
# swe-1-7), at model level. A fresh 429/concurrency signal halves the
# learned cap and benches until the provider window.
# hard_ceiling rows (ollama) never probe. Metered rows default
...
# fleet-ops#3125: rolling last-20-sessions PR-yield ledger per seat, written
# each tick by libexec/fleet-metrics-export.py. pick_seat consults it for
# product-role picks (PI_PICK_ROLE=product) when seat-caps product_order is
# "yield"; a seat absent from the ledger or with <20 measured sessions gets
# provisional yield 0.5 so new seats are tried, not starved.
SEAT_YIELD_JSON="${SEAT_YIELD_JSON:-$STATE_DIR/seat-yield.json}"
...
load_seat_caps() {
    ...
    while IFS=$'\t' read -r p m cap class mprobe; do
        [[ -n "$p" && -n "$m" ]] || continue
    ...
    done < <(jq -r '.providers | to_entries[] | .key as $p | .value as $v | (if ($v|type)=="object" then ($v.models // {}) else {} end) | to_entries[] | [$p, .key, (.value // 0 | tostring), (if (.value|type)=="object" then (.value.class // "") else "" end), (if (.value|type)=="object" then (.value.max_probe_ceiling // "") else "" end)] | @tsv' "$SEAT_CAPS_JSON" 2>/dev/null || true)
...
# next to "devin" without collision. fleet-ops-deploy resets learned-caps.json
# when a deploy changes seat-caps.json so a stale learned cap never pins a
...
# retry. heavy/light (default) keep the class walk — free first, then
# prepaid, then metered; PI_PICK_ROLE=product + product_order=yield reranks
# the same candidates by the rolling PR-yield ledger (fleet-ops#3125).
...
pick_seat() {
    ...
    # seat by the rolling last-20-sessions PR yield in seat-yield.json,
    # descending; ties break by class (prepaid-quota -> metered -> free, so
    # prepaid subs still drain first among equal performers) then by each
    # bucket's existing order. A seat absent from the ledger or with fewer
    # than 20 measured sessions gets provisional yield 0.5 — new seats are
    # tried, not starved. Keystone and non-product picks keep the class
    # ladder below.
    local -a product_seats=()
    if ! _is_keystone_class "$difficulty" \
        && [[ "${PI_PICK_ROLE:-scout}" == "product" ]] \
        && [[ "$SEAT_PRODUCT_ORDER" == "yield" ]]; then
        declare -A _pick_yield=()
        local _yk _yv
        if [[ -f "$SEAT_YIELD_JSON" ]]; then
            while IFS=$'\t' read -r _yk _yv; do
                [[ -n "$_yk" ]] || continue
                _pick_yield["$_yk"]="$_yv"
            done < <(jq -r '(.seats // {}) | to_entries[] | [.key, ((if ((.value.sessions // 0) >= 20) then (.value.yield // 0.5) else 0.5 end) | tostring)] | @tsv' "$SEAT_YIELD_JSON" 2>/dev/null || true)
        fi
        local -a _yranked=()
        mapfile -t _yranked < <(
            _fm=""
