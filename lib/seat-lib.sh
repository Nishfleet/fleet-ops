...
# fleet-ops#3322: audition-lane flags. A seat carrying audition: true is a
# candidate under measurement (cap 1, light-only). Keyed on "provider" for a
# provider-level audition flag, "provider/model" for a model-level flag.
...
_seat_floor_shortest_bench() {
    ...
    while IFS=$'\t' read -r p m _ capable; do
        ...
        # fleet-ops#3322: the light-only audition gate binds the floor too —
        # a fail-open heavy pick must not lift an audition seat's bench.
        if [[ "$difficulty" != "light" ]] \
            && [[ -n "${SEAT_MODEL_AUDITION[$p/$m]:-}" \
                  || -n "${SEAT_PROVIDER_AUDITION[$p]:-}" ]]; then
            continue
        fi
...
is_quota_cap_error() {
    ...
    if grep -qiE 'weekly[[:space:]]+(clinepass[[:space:]]+)?limit|daily[[:space:]]+limit|INFERENCE_CAP_ERROR|FreeUsageLimitError|usage[[:space:]]+balance[[:space:]]+exhausted|usage[[:space:]]+limit[[:space:]]+for[[:space:]]+the[[:space:]]+current[[:space:]]+free[[:space:]]+model|free-model[[:space:]]+token[[:space:]]+quota|resource_exhausted' <<<"$combined"; then
        return 0
    fi
    return 1
}
