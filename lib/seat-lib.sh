...
seat_usable() {
    ...
    f=$(seat_ledger_path "$p" "$m")
    if [[ ! -f "$f" ]]; then
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: NO HEALTH DATA (no ledger file) — assuming usable"
        return 0
    fi
    ...
    if [[ -z "$hc" ]]; then
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: NO HEALTH DATA (ledger unparseable) — assuming usable"
        return 0
    fi
    ...
    if [[ "$hc" == "quota_bench" ]]; then
        if [[ -n "$bench_until" ]] && _seat_in_future "$bench_until"; then
            (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: benched until $bench_until (quota_bench)"
            return 1
        fi
        ...
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: UNUSABLE (quota_bench with no bench_until — defensive block)"
        return 1
    fi
    ...
    if [[ "$hc" == "overload_bench" ]]; then
        if [[ -n "$bench_until" ]] && _seat_in_future "$bench_until"; then
            (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: benched until $bench_until (overload_bench)"
            return 1
        fi
        if [[ -n "$bench_until" ]]; then
            seat_log "seat $p/$m: bench expired ($bench_until passed) — assuming usable (fail-open)"
        ...
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: UNUSABLE (overload_bench with no bench_until — defensive block)"
        return 1
    fi
    ...
    if [[ "$hc" == "hang_bench" ]]; then
        if [[ -n "$bench_until" ]] && _seat_in_future "$bench_until"; then
            (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: benched until $bench_until (hang_bench)"
            return 1
        fi
        if [[ -n "$bench_until" ]]; then
        ...
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: UNUSABLE (hang_bench with no bench_until — defensive block)"
        return 1
    fi
    ...
    if [[ "$dead" == "true" ]]; then
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: UNUSABLE (seat_dead=true, class=$hc)"
        return 1
    fi
    if [[ "$hc" == "quota_exhausted" || "$hc" == "credentials_bad" ]]; then
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: UNUSABLE (health_class=$hc)"
        return 1
    fi
    ...
    if [[ "$hc" == "rate_limited" ]]; then
        if _seat_rate_limit_fresh "$observed" && [[ -n "$usable_at" ]] && _seat_in_future "$usable_at"; then
            (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: UNUSABLE (rate_limited until $usable_at, observed ${observed:-<empty>})"
            return 1
        fi
    ...
    if [[ -n "$usable_at" ]] && _seat_in_future "$usable_at"; then
        (( _SEAT_USABLE_SILENT )) || seat_log "seat $p/$m: UNUSABLE (backoff until $usable_at, class=$hc)"
        return 1
    fi
    return 0
}
