...
# from worker_memory.<repo> (0509 1536M=1.5G, fleet-ops 1G), converted to
# GB. Repos without a row fall back to ram_gb_per_worker (0.5). This is
# what admission charges each active worker, so a 0509 browser worker
# consumes its real 1.5 GB share of MemAvailable instead of the flat
# 0.5 GB (fleet-ops#3679).
ram_charge_gb_for() {
...
# fallback, so a 0509 browser worker (1.5 GB) charges 3 units and a
# fleet-ops worker (1.0 GB) charges 2 units instead of the flat 1
...
is_quota_cap_error() {
    ...
    if grep -qiE 'weekly[[:space:]]+(clinepass[[:space:]]+)?limit|daily[[:space:]]+limit|INFERENCE_CAP_ERROR|FreeUsageLimitError|usage[[:space:]]+limit[[:space:]]+for[[:space:]]+the[[:space:]]+current[[:space:]]+free[[:space:]]+model|free-model[[:space:]]+token[[:space:]]+quota|resource_exhausted' <<<"$combined"; then
        return 0
    fi
    return 1
}
