...
DEFAULT_TARGETS = [
    {
        ...
        "failure_response_block": "- **Failure response is #1 (Nish, 2026-08-08):** any detected fleet-infrastructure failure gets automatic, autonomous, INSTANT repair dispatch - trace line in the console actions.log, then the VPS repair ladder (cheapest healthy Pi seat for deeply-scoped fixes, flagship Pi seats for broad or high-stakes — pick seats via `agent-state/lanes/pi-seat-health.json`; one live flagship, throttled, fail-loud when repair is impossible). Never a quiet degraded mode, never 'flag for Nish'. Mac agents dispatch to the VPS, never repair locally. Canonical text: vault memory `failure-response-standing-order`.\n\n",
    },
    {
