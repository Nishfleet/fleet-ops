...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--targets",
        help="comma-separated target specs: path|surface|label (overrides defaults)",
    )
    ...
    if args.targets:
        ...
        for spec in args.targets.split(","):
            path_str, surface, label, phrase = spec.split("|", 3)
            targets.append(
                {
                    "path": Path(path_str),
                    "surface_agent": surface,
                    "agent_label": label,
                    "preimplement_phrase": phrase,
                }
            )
    else:
