...
dir. This is a pre-pass for the review conference, not an alert: nothing
pages on a high z-score.

Heartbeat: writes fleet-baseline-delta.prom so FleetBaselineDeltaAbsent /
FleetBaselineDeltaStale can notice a dead organ. The ranked list itself
never becomes a Prometheus alert.
...
def daily_fallback(
    ...
    """When weekly history is too short: this period vs prior daily samples.

    If the retained window is shorter than 7d, "this" is the last day and
    earlier days are the baseline — otherwise every sample sits inside
    "this week" and the report would stay empty until retention grows.
    """
    if not points:
        return None, []
    points = sorted(points)
    span = now - points[0][0]
    this_start = now - 86400 if span <= WEEK else now - WEEK
    if kind == "counter":
        this = [p for p in points if p[0] > this_start]
        prior = [p for p in points if p[0] <= this_start]
        this_val = (this[-1][1] - this[0][1]) if len(this) >= 2 else None
        prior_vals: list[float] = []
        by_day: dict[int, list[tuple[float, float]]] = {}
        for ts, val in prior:
            by_day.setdefault(int(ts // 86400), []).append((ts, val))
        for day in sorted(by_day):
            samples = sorted(by_day[day])
            if len(samples) >= 2:
                prior_vals.append(samples[-1][1] - samples[0][1])
        return this_val, prior_vals
    this = [v for ts, v in points if ts > this_start]
    prior = [v for ts, v in points if ts <= this_start]
    if not this:
        this = [points[-1][1]]
        prior = [v for ts, v in points[:-1]]
    this_val = statistics.median(this) if this else None
    by_day_g: dict[int, list[float]] = {}
    for ts, val in points:
        if ts <= this_start:
            by_day_g.setdefault(int(ts // 86400), []).append(val)
    prior_vals = [statistics.median(vs) for vs in by_day_g.values()]
    if len(prior_vals) < 2 and prior:
        prior_vals = prior
    return this_val, prior_vals
...
def score_series(
    ...
    if this_week is None or len(baseline) < 2:
        this_week, baseline = daily_fallback(points, now, kind)
        grain = "daily-warmup"
    if this_week is None or len(baseline) < 2:
        return {
            "id": series_id(metric),
...
def collect_live(url: str, now: float, timeout: float, z_thresh: float) -> list[dict[str, Any]]:
    ...
    wanted = [n for n in names if n.startswith("fleet_") or n in KEY_NODE]
    if not wanted:
...
def render_report(
    ...
    if not ranked:
        ...
        lines.append("| rank | |z| | z | this | median | MAD | grain | series |")
        lines.append("| ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |")
        for i, row in enumerate(ranked, 1):
            ...
            lines.append(
                ...
                f"{row['median']:.6g} | {row['mad']:.6g} | {row['grain']} | `{row['id']}` |"
            )
        lines.append("")
    payload = {
        "generated": iso,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.fixture:
            ...
            grains = {s.get("grain") for s in scored if s.get("grain")}
            grain_note = ",".join(sorted(grains)) if grains else "fixture"
        else:
            scored = collect_live(args.prom_url, now, args.timeout, args.z_threshold)
            grains = {s.get("grain") for s in scored if s.get("grain")}
            grain_note = ",".join(sorted(g for g in grains if g)) or "weekly"
        md, payload, ranked = render_report(
            scored, now, args.z_threshold, args.top, args.prom_url, grain_note
        )
