...
def find_checkout() -> Path:
    ...
    # We are installed as a symlink into the repo's bin/ directory.
...
def parse_manifest(checkout: Path) -> tuple[dict[str, Path], set[str]]:
    """Return (src->dest mapping, set of unit names that should be enabled)."""
    ...
    with manifest.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            # Only user-scope systemd units are enabled.
            if not dest.startswith(user_systemd_dir):
                continue

            basename = os.path.basename(dest)
            # Template instances are derived from intake-repos.json separately.
            if "@" in basename:
                continue
...
def check_enabled_units(checkout: Path, expected_enabled: set[str]) -> None:
    ...
    # Expected units must be enabled.
    missing: list[str] = []
    ...
    # Extra enabled fleet units.
    rc, out, _ = run([SYSTEMCTL, "--user", "list-unit-files", "--state=enabled", "--no-legend", "--plain"], check=False)
    if rc != 0:
...
def check_extra_symlinks(checkout: Path, expected_dests: set[str]) -> None:
    ...
    for d in MANAGED_DIRS:
        ...
        for item in d.iterdir():
            ...
            # If the symlink points into the current checkout, it is a hand-
            # installed extra from this checkout (file exists but is not in
            # MANIFEST).
            if target_str.startswith(checkout_str + os.sep):
                findings.append(f"{item} -> {target} (extra from current checkout)")
                continue

            # If it points at another fleet-ops checkout or is a stale symlink,
            # it is drift.
            if is_fleet_path(target) or is_fleet_path(item):
                findings.append(f"{item} -> {target} (outside current checkout)")
                continue

            # A systemd user unit symlink with a fleet-looking name is an extra
            # even if the target is not identifiable as a fleet-ops path.
